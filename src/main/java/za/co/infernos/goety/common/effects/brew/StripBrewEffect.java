...
public class StripBrewEffect extends BrewEffect {
    ...
    public void applyEntityEffect(LivingEntity pTarget, @Nullable Entity pSource, @Nullable Entity pIndirectSource, int pAmplifier){
        for (int i = 0; i < pAmplifier + 1; ++i){
            ...
            int random = pTarget.getRandom().nextInt(4);
            if (random == 0) equipmentSlot = EquipmentSlot.FEET;
            if (random == 1) equipmentSlot = EquipmentSlot.LEGS;
            if (random == 2) equipmentSlot = EquipmentSlot.CHEST;
            ...
            if (!armor.isEmpty()){
                int binding = net.minecraft.world.item.enchantment.EnchantmentHelper.getItemEnchantmentLevel(pTarget.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(net.minecraft.world.item.enchantment.Enchantments.BINDING_CURSE), armor);
                if (binding > 0){
                    armor = ItemStack.EMPTY;
                }
            }
