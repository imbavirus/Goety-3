...
import za.co.infernos.goety.client.particles.PulsatingCircleParticleOption;
...
public class ScatterMine extends Entity implements ISpellEntity {
    ...
    public void explodeDamage(LivingEntity livingEntity) {
        if (!this.level().isClientSide) {
            ...
            float damage = za.co.infernos.goety.utils.ConfigHelper.getFloat(SpellConfig.ScatterMineDamage, 1.0F);
            if (!this.isSpell()) {
