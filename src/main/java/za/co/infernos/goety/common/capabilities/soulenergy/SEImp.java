...
public class SEImp implements ISoulEnergy{
    ...
    public boolean increaseSE(int increase) {
        if (this.soulEnergy >= za.co.infernos.goety.utils.ConfigHelper.getInt(MainConfig.MaxArcaSouls, 0)) {
            return false;
        }
        this.soulEnergy = Math.min(this.soulEnergy + increase, za.co.infernos.goety.utils.ConfigHelper.getInt(MainConfig.MaxArcaSouls, 0));
        return true;
    }
