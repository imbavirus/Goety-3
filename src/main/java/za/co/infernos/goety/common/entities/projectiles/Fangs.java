...
public class Fangs extends Entity implements ISpellEntity {
    ...
    private void dealDamageTo(LivingEntity target) {
        ...
        float baseDamage = za.co.infernos.goety.utils.ConfigHelper.getFloat(SpellConfig.FangDamage, 1.0F) * WandUtil.damageMultiply();
                ...
                if (livingentity instanceof Player player) {
                    if (this.isTotemSpawned()) {
                        ...
                        if (target.hurt(this.damageSources().indirectMagic(this, livingentity), baseDamage)) {
                            ...
                            SEHelper.increaseSouls(player, za.co.infernos.goety.utils.ConfigHelper.getInt(SpellConfig.FangGainSouls, 0) * soulEater);
                            if (this.burning > 0) {
                                target.igniteForSeconds(5 * this.burning);
                            }
                        }
