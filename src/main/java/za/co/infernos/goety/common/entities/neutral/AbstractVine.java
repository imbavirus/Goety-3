                ...
                if (!this.isOnFire() && !this.isDeadOrDying() && (!this.limitedLifespan || this.limitedLifeTicks > 20)) {
                    if (this.getHealth() < this.getMaxHealth()){
                        if (this.getTrueOwner() instanceof Player owner) {
                            ...
                            int healRate = 0;
                            float healAmount = 0;
                            if (za.co.infernos.goety.utils.ConfigHelper.getBoolean(MobsConfig.NaturalMinionHeal, false)){
                                curio = CuriosFinder.hasWildRobe(owner);
                                soulCost = za.co.infernos.goety.utils.ConfigHelper.getInt(MobsConfig.NaturalMinionHealCost, 0);
                                healRate = za.co.infernos.goety.utils.ConfigHelper.getInt(MobsConfig.NaturalMinionHealTime, 0);
                                healAmount = za.co.infernos.goety.utils.ConfigHelper.getFloat(MobsConfig.NaturalMinionHealAmount, 1.0F);
                            }
                            if (curio) {
