package za.co.infernos.goetied.api.items.armor;
...
public interface ISoulDiscount {
    ...
    default Component soulDiscountTooltip(ItemStack itemStack){
        ...
        if (discount > 0) {
            return Component.literal(String.valueOf(discount)).append("% ").append(Component.translatable("info.goetied.armor.discount")).withStyle(ChatFormatting.DARK_AQUA);
        } else {
