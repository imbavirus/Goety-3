package za.co.infernos.goetied.api.magic;
...
public interface ISummonSpell extends ISpell{
    ...
    default boolean hasSummonDown(LivingEntity caster){
        return caster.hasEffect(GoetiedEffects.SUMMON_DOWN);
    }
    ...
    default void SummonSap(LivingEntity owner, LivingEntity summonedEntity){
        if (owner != null && summonedEntity != null) {
            if (this.hasSummonDown(owner)) {
                MobEffectInstance effectinstance = owner.getEffect(GoetiedEffects.SUMMON_DOWN);
                if (effectinstance != null) {
                    ...
                    summonedEntity.addEffect(new MobEffectInstance(GoetiedEffects.SAPPED, EffectsUtil.infiniteEffect(), effectinstance.getAmplifier() + 4));
                }
    ...
    default void SummonDown(LivingEntity entityLiving){
        MobEffectInstance effectinstance1 = entityLiving.getEffect(GoetiedEffects.SUMMON_DOWN);
        int i = 1;
        if (effectinstance1 != null) {
            i += effectinstance1.getAmplifier();
            entityLiving.removeEffectNoUpdate(GoetiedEffects.SUMMON_DOWN);
        } else {
            --i;
        }
        ...
        MobEffectInstance effectinstance = new MobEffectInstance(GoetiedEffects.SUMMON_DOWN, s, i, false, false, true);
