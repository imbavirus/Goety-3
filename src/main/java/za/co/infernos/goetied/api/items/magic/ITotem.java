package za.co.infernos.goetied.api.items.magic;
...
public interface ITotem {
    ...
    static int getDefaultMaxSouls() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.MaxSouls, 10000);
    }
    ...
    static boolean UndyingEffect(Player player){
        ...
        if (!itemStack.isEmpty()) {
            CustomData data = itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
            if (!data.isEmpty() && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.TotemUndying, false)) {
