package za.co.infernos.goetied.client.events;

import za.co.infernos.goetied.Goetied;
import za.co.infernos.goetied.api.blocks.entities.IBarrack;
import za.co.infernos.goetied.api.blocks.entities.IOwnedBlock;
import za.co.infernos.goetied.api.blocks.entities.ITrainingBlock;
import za.co.infernos.goetied.api.blocks.entities.IWaystoneBlock;
...
import za.co.infernos.goetied.client.audio.*;
import za.co.infernos.goetied.client.gui.screen.inventory.BrewRadialMenuScreen;
import za.co.infernos.goetied.client.gui.screen.inventory.FocusRadialMenuScreen;
import za.co.infernos.goetied.client.render.BurrowingLaserRenderer;
import za.co.infernos.goetied.client.render.GuardianLaserRenderer;
import za.co.infernos.goetied.client.render.ModModelLayer;
import za.co.infernos.goetied.client.render.WearRenderer;
import za.co.infernos.goetied.client.render.item.CustomItemsRenderer;
import za.co.infernos.goetied.client.render.model.LichModeModel;
import za.co.infernos.goetied.common.blocks.entities.ArcaBlockEntity;
import za.co.infernos.goetied.common.blocks.entities.BrewCauldronBlockEntity;
import za.co.infernos.goetied.common.blocks.entities.CursedCageBlockEntity;
import za.co.infernos.goetied.common.blocks.entities.OminousIdolBlockEntity;
import za.co.infernos.goetied.common.effects.GoetiedEffects;
import za.co.infernos.goetied.common.entities.ally.GuardianServant;
import za.co.infernos.goetied.common.entities.ally.Leapleaf;
import za.co.infernos.goetied.common.entities.ally.golem.SquallGolem;
...
import za.co.infernos.goetied.common.entities.hostile.Wight;
import za.co.infernos.goetied.common.entities.hostile.ender.Endersent;
import za.co.infernos.goetied.common.entities.hostile.illagers.HostileRedstoneGolem;
import za.co.infernos.goetied.common.entities.hostile.illagers.HostileRedstoneMonstrosity;
...
import za.co.infernos.goetied.common.entities.neutral.ApostleShade;
import za.co.infernos.goetied.common.entities.neutral.CarrionFly;
import za.co.infernos.goetied.common.entities.neutral.InsectSwarm;
import za.co.infernos.goetied.common.entities.neutral.Wildfire;
import za.co.infernos.goetied.common.entities.projectiles.CorruptedBeam;
import za.co.infernos.goetied.common.entities.projectiles.IceStorm;
import za.co.infernos.goetied.common.entities.util.CameraShake;
import za.co.infernos.goetied.common.items.WaystoneItem;
import za.co.infernos.goetied.common.items.curios.GloveItem;
import za.co.infernos.goetied.common.magic.spells.abyss.PrismaBeamSpell;
import za.co.infernos.goetied.common.magic.spells.geomancy.BurrowingSpell;
import net.neoforged.neoforge.network.PacketDistributor;
import za.co.infernos.goetied.common.network.ModNetwork;
import za.co.infernos.goetied.common.network.client.*;
...
@EventBusSubscriber(modid = Goetied.MOD_ID, bus = EventBusSubscriber.Bus.GAME, value = Dist.CLIENT)
public class ClientEvents {
    ...
    public static void onSetupCamera(ViewportEvent.ComputeCameraAngles event) {
        ...
        if (player != null) {
            ...
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.CameraShake, false) && !Minecraft.getInstance().isPaused()) {
    ...
    public static void onEntityTick(EntityTickEvent.Post event){
        ...
        if (entity.level() instanceof ClientLevel){
            ...
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.BossMusic, false)) {
    ...
    public static void renderGlove(RenderArmEvent event){
        if (event.isCanceled() || !za.co.infernos.goetied.utils.ConfigHelper.getBoolean(ItemConfig.FirstPersonGloves, false)){
            return;
        }
    ...
    public static void renderArm(RenderArmEvent event){
        ...
        if (!player.isSpectator() && LichdomHelper.isInLichMode(player)){
            ...
            final ResourceLocation texture = Goetied.location("textures/entity/lich.png");
        ...
        if (event.getPlayer().hasEffect(GoetiedEffects.SHADOW_WALK)){
                    ...
                    if (blockEntity instanceof ArcaBlockEntity arcaTile) {
                        if ((player.isShiftKeyDown() || player.isCrouching())) {
                            if (arcaTile.getPlayer() == player && SEHelper.getSEActive(player)) {
                                ...
                                int SoulEnergyTotal = za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.MaxArcaSouls, 0);
                                String s = Component.translatable("tooltip.goetied.blockSoul").getString() + SoulEnergy + "/" + SoulEnergyTotal;
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s = Component.translatable("tooltip.goetied.blockOwner").getString() + arcaTile.getPlayer().getDisplayName().getString();
                                int l = fontRenderer.width(s);
                                event.getGuiGraphics().drawString(fontRenderer, s, (-l / 2), -4, 0xFFFFFF);
                                RenderSystem.disableBlend();
                                poseStack.popPose();
                        ...
                        if (owner != null) {
                            if (blockEntity instanceof ITrainingBlock trainingBlock) {
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s = Component.translatable("tooltip.goetied.blockOwner").getString() + owner.getDisplayName().getString();
                                int l = fontRenderer.width(s);
                                event.getGuiGraphics().drawString(fontRenderer, s, (-l / 2), -4, 0xFFFFFF);
                                RenderSystem.disableBlend();
                                poseStack.popPose();
                                ...
                                if (owner == player) {
                                    if (trainingBlock.reachedLimit()) {
                                        ...
                                        RenderSystem.enableBlend();
                                        RenderSystem.defaultBlendFunc();
                                        String s0 = Component.translatable("info.goetied.summon.limit").getString();
                                    ...
                                    RenderSystem.enableBlend();
                                    RenderSystem.defaultBlendFunc();
                                    String mode = Component.translatable("tooltip.goetied.blockGuard").getString();
                                    if (!trainingBlock.isGuarding()) {
                                        mode = Component.translatable("tooltip.goetied.blockFollow").getString();
                                    }
                                    ...
                                    if (trainingBlock.isSensorSensitive()) {
                                        ...
                                        RenderSystem.enableBlend();
                                        RenderSystem.defaultBlendFunc();
                                        String s0 = Component.translatable("tooltip.goetied.blockSense").getString();
                                        int l0 = fontRenderer.width(s0);
                                        event.getGuiGraphics().drawString(fontRenderer, s0, (-l0 / 2), -4, 0xFFFFFF);
                                        RenderSystem.disableBlend();
                                        poseStack.popPose();
                                    ...
                                    if (trainingBlock.isGrounding()) {
                                        ...
                                        RenderSystem.enableBlend();
                                        RenderSystem.defaultBlendFunc();
                                        String s0 = Component.translatable("tooltip.goetied.blockGrounded").getString();
                                        int l0 = fontRenderer.width(s0);
                                        event.getGuiGraphics().drawString(fontRenderer, s0, (-l0 / 2), -4, 0xFFFFFF);
                                        RenderSystem.disableBlend();
                                        poseStack.popPose();
                                    ...
                                    RenderSystem.enableBlend();
                                    RenderSystem.defaultBlendFunc();
                                    String s1 = Component.translatable("tooltip.goetied.blockTrain").getString() + trainingBlock.amountTrainLeft() + "/" + trainingBlock.maxTrainAmount() + " " + trainingBlock.getTrainMob().getDescription().getString();
                                    ...
                                    event.getGuiGraphics().blit(Goetied.location("textures/gui/train_bar.png"), ((width - 64) / 2), (height - 86), 0, 0, 64, 16, 64, 32);
                                    event.getGuiGraphics().blit(Goetied.location("textures/gui/train_bar.png"), ((width - 64) / 2), (height - 86), 0, 16, train, 16, 64, 32);
                                    poseStack.popPose();
                                }
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s1 = Component.translatable("tooltip.goetied.blockTrainType").getString() + Component.translatable(barrack.getCurrentMob()).getString();
                                int l1 = fontRenderer.width(s1);
                                event.getGuiGraphics().drawString(fontRenderer, s1, (-l1 / 2), -4, 0xFFFFFF);
                                RenderSystem.disableBlend();
                                poseStack.popPose();
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s2 = Component.translatable("tooltip.goetied.brew.capacity").getString() + barrack.getCurrentAmount() + "/" + barrack.trainLimit();
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s2 = Component.translatable("tooltip.goetied.idol.count").getString() + idol.getClientCount() + "/" + za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.OminousIdolLimit, 0);
                                int l2 = fontRenderer.width(s2);
                                event.getGuiGraphics().drawString(fontRenderer, s2, (-l2 / 2), -4, 0xFFFFFF);
                                RenderSystem.disableBlend();
                                poseStack.popPose();
                        ...
                        if (player.isShiftKeyDown() || player.isCrouching() && !cageBlockEntity.getItem().isEmpty()){
                            ...
                            RenderSystem.enableBlend();
                            RenderSystem.defaultBlendFunc();
                            String s = Component.translatable("tooltip.goetied.blockSoul").getString() + cageBlockEntity.getSouls();
                            int l = fontRenderer.width(s);
                            event.getGuiGraphics().drawString(fontRenderer, s, (-l / 2), -4, 0xFFFFFF);
                            RenderSystem.disableBlend();
                            poseStack.popPose();
                        ...
                        if (player.isShiftKeyDown() || player.isCrouching()){
                            ...
                            RenderSystem.enableBlend();
                            RenderSystem.defaultBlendFunc();
                            String s1 = Component.translatable("tooltip.goetied.brew.capacity").getString() + cauldronBlock.getCapacityUsed() + "/" + cauldronBlock.getCapacity();
                            ...
                            RenderSystem.enableBlend();
                            RenderSystem.defaultBlendFunc();
                            String s = Component.translatable("tooltip.goetied.blockSoulCost").getString() + (cauldronBlock.getBrewCost() - cauldronBlock.soulTime);
                            int l = fontRenderer.width(s);
                            event.getGuiGraphics().drawString(fontRenderer, s, (-l / 2), -4, 0xFFFFFF);
                            RenderSystem.disableBlend();
                            poseStack.popPose();
                        ...
                        if ((player.isShiftKeyDown() || player.isCrouching()) && globalPos != null) {
                            ...
                            String s1 = Component.translatable("tooltip.goetied.arcaCoords", blockPos.getX(), blockPos.getY(), blockPos.getZ()).getString();
                            int l2 = fontRenderer.width(s1);
                            event.getGuiGraphics().drawString(fontRenderer, s1, (-l2 / 2), -4, 0xFFFFFF);
                            RenderSystem.disableBlend();
                            poseStack.popPose();
                            ...
                            RenderSystem.enableBlend();
                            RenderSystem.defaultBlendFunc();
                            String s = Component.translatable("tooltip.goetied.arcaDimension", globalPos.dimension().location().toString()).getString();
                            int l = fontRenderer.width(s);
                            event.getGuiGraphics().drawString(fontRenderer, s, (-l / 2), -4, 0xFFFFFF);
                            RenderSystem.disableBlend();
                            poseStack.popPose();
                            if (waystoneBlock.getSoulCost() > 0) {
                                ...
                                RenderSystem.enableBlend();
                                RenderSystem.defaultBlendFunc();
                                String s0 = Component.translatable("tooltip.goetied.blockSoulCost").getString() + waystoneBlock.getSoulCost();
                                int l0 = fontRenderer.width(s0);
                                event.getGuiGraphics().drawString(fontRenderer, s0, (-l0 / 2), -4, 0xFFFFFF);
                                RenderSystem.disableBlend();
                                poseStack.popPose();
    ...
    public static void updateInputEvent(MovementInputUpdateEvent event) {
        ...
        if (player instanceof LocalPlayer localPlayer) {
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.WheelGuiMovement, false)) {
            ...
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(SpellConfig.FullStopCast, false)) {
    ...
    public static void KeyInputs(InputEvent.Key event) {
        ...
        if (ModKeybindings.keyBindings[0].isDown() && MINECRAFT.isWindowActive()){
            Goetied.LOGGER.debug("[Goetied] Z pressed -> sending CWandKeyPacket");
        ...
        if (ModKeybindings.keyBindings[6].isDown() && MINECRAFT.isWindowActive()){
            ...
            if (MINECRAFT.player != null && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.LichNightVision, false)){
