...
public class SoulEnergyGui {
    ...
    public static final ResourceLocation LAYER_ID = Goetied.location("soul_energy_hud");
    ...
    public static boolean shouldDisplayBar() {
        return SEHelper.getSoulsContainer(minecraft.player) && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.SoulGuiShow, false)
    ...
    public static void drawHUD(GuiGraphics guiGraphics, DeltaTracker partialTick, int screenWidth, int screenHeight) {
        ...
        int SoulEnergy = 0;
        int SoulEnergyTotal = za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.MaxSouls, 0);
        if (SEHelper.getSEActive(minecraft.player)) {
            SoulEnergy = SEHelper.getSESouls(minecraft.player);
            SoulEnergyTotal = za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.MaxArcaSouls, 0);
        ...
        int i = (screenWidth / 2) + (za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.SoulGuiHorizontal, 0));
        int energylength = (int) (117 * (SoulEnergy / (double) SoulEnergyTotal));
        int maxenergy = (int) (117 * (za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.MaxSouls, 0) / (double) SoulEnergyTotal));

        int height = screenHeight + (za.co.infernos.goetied.utils.ConfigHelper.getInt(MainConfig.SoulGuiVertical, 0));

        int offset = (int) ((minecraft.player.tickCount + partialTick.getGameTimeDeltaPartialTick(false)) % 234);

        if (SEHelper.getSEActive(minecraft.player)) {
            guiGraphics.blit(Goetied.location("textures/gui/soul_energy.png"), i, height - 9, 0, 9, 128, 9, 128, 90);
            guiGraphics.blit(Goetied.location("textures/gui/soul_energy.png"), i + 9, height - 9, 9, 18, maxenergy, 9,
                    128, 90);
        } else {
            int height1 = stack.getItem() instanceof FullSpentTotem ? 36 : 0;
            guiGraphics.blit(Goetied.location("textures/gui/soul_energy.png"), i, height - 9, 0, height1, 128, 9, 128,
                    90);
        }
        RenderSystem.setShaderTexture(0, Goetied.location("textures/gui/soul_energy_bar.png"));
        guiGraphics.blit(Goetied.location("textures/gui/soul_energy_bar.png"), i + 9, height - 7, offset, 0, energylength,
                5, 234, 5);
        ...
        if (MobUtil.isSpellCasting(minecraft.player)) {
            ...
            guiGraphics.blit(Goetied.location("textures/gui/soul_energy.png"), i + 9, height - 9, 9, bar, useTime, 9, 128,
                    90);
        }

        if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.ShowNum, false)) {
