...
public class PlayerSoulArmorLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    private static final ResourceLocation SOUL_ARMOR = ResourceLocation.fromNamespaceAndPath(Goetied.MOD_ID, "textures/entity/soul_armor.png");
    ...
    public void render(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight, T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTicks, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        if (pLivingEntity.hasEffect(GoetiedEffects.SOUL_ARMOR) && pLivingEntity.hurtTime > 0) {
