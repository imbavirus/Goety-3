...
import za.co.infernos.goetied.client.render.model.EntangleVinesModel;
import za.co.infernos.goetied.common.entities.projectiles.SnarelingGoop;
...
public class SnarelingGoopRenderer<T extends SnarelingGoop> extends EntityRenderer<T> {
    private static final ResourceLocation TEXTURE_LOCATION = Goetied.location("textures/entity/projectiles/snareling_goop.png");
    private final EntangleVinesModel<T> model;

    public SnarelingGoopRenderer(EntityRendererProvider.Context p_i47208_1_) {
        super(p_i47208_1_);
    ...
    public void render(T pEntity, float entityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        VertexConsumer ivertexbuilder = bufferIn.getBuffer(ModRenderType.wraith(this.getTextureLocation(pEntity)));
