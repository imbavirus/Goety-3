...
import za.co.infernos.goetied.client.render.model.VoidShockModel;
...
public class VoidShockRenderer extends EntityRenderer<VoidShock> {
    private static final ResourceLocation OUTER_TEXTURES = Goetied
            .location("textures/entity/projectiles/void_shock_outer.png");
    private static final ResourceLocation INNER_TEXTURES = Goetied
            .location("textures/entity/projectiles/void_shock_inner.png");
    private static final ResourceLocation TRAIL_TEXTURE = Goetied.location("textures/particle/trail.png");
    ...
    private void renderTrail(VoidShock entityIn, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn,
        ...
        while (samples < sampleSize) {
            ...
            vertexconsumer
                    ...
                    .setColor(red, green, blue, alpha).setUv(f2, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY)
                    .setLight(packedLightIn).setNormal(0.0F, 1.0F, 0.0F);
            vertexconsumer
                    ...
                    .setColor(red, green, blue, alpha).setUv(f3, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY)
                    .setLight(packedLightIn).setNormal(0.0F, 1.0F, 0.0F);
            vertexconsumer
                    ...
                    .setColor(red, green, blue, alpha).setUv(f3, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY)
                    .setLight(packedLightIn).setNormal(0.0F, 1.0F, 0.0F);
            vertexconsumer
                    .addVertex(matrix4f, (float) draw1.x + (float) topAngleVec.x, (float) draw1.y + (float) topAngleVec.y,
                            (float) draw1.z + (float) topAngleVec.z)
                    .setColor(red, green, blue, alpha).setUv(f2, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY)
                    .setLight(packedLightIn).setNormal(0.0F, 1.0F, 0.0F);
            samples++;
            drawFrom = sample;
        }
