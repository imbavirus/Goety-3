...
public class ZPiglinBandsLayer<T extends Summoned> extends RenderLayer<T, ZPiglinModel<T>> {
    private static final ResourceLocation TEXTURES = Goetied.location("textures/entity/servants/zombie/zpiglin_servant_bands.png");
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn.getTrueOwner() instanceof Player && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.ZPiglinServantTexture, false)) {
