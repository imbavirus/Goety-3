...
import za.co.infernos.goetied.common.entities.projectiles.NecroBolt;
...
public class NecroBoltRenderer<T extends NecroBolt> extends EntityRenderer<T> {
    public static final ResourceLocation BASE = Goetied.location("textures/particle/necro_bolt_base.png");
    public static final ResourceLocation MID = Goetied.location("textures/particle/necro_bolt_mid.png");
    public static final ResourceLocation TOP = Goetied.location("textures/particle/necro_bolt_top.png");
    private static final ResourceLocation TRAIL = Goetied.location("textures/entity/projectiles/trail.png");
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, NecroBolt projectile, float partialTicks, int packedLightIn, Matrix4f matrix4f, Matrix3f matrix3f, int mode) {
        ...
        this.vertex(vertexConsumer, matrix4f, avector3f[0].x, avector3f[0].y, avector3f[0].z).setColor(255, 255, 255, 255).setUv(0, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
        this.vertex(vertexConsumer, matrix4f, avector3f[1].x, avector3f[1].y, avector3f[1].z).setColor(255, 255, 255, 255).setUv(1, 1).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
        this.vertex(vertexConsumer, matrix4f, avector3f[2].x, avector3f[2].y, avector3f[2].z).setColor(255, 255, 255, 255).setUv(1, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
        this.vertex(vertexConsumer, matrix4f, avector3f[3].x, avector3f[3].y, avector3f[3].z).setColor(255, 255, 255, 255).setUv(0, 0).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
    }
    ...
    private void renderTrail(NecroBolt entityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, float red, float green, float blue, float alpha, int packedLightIn) {
        ...
        while (samples < sampleSize) {
            ...
            float f3 = f2 + 0.5F;
            Vector3f n = new Vector3f(0.0F, 1.0F, 0.0F).mul(matrix3f);
            this.vertex(vertexconsumer, matrix4f, (float) draw1.x + (float) bottomAngleVec.x, (float) draw1.y + (float) bottomAngleVec.y, (float) draw1.z + (float) bottomAngleVec.z).setColor(red, green, blue, alpha).setUv(f2, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
            this.vertex(vertexconsumer, matrix4f, (float) sample.x + (float) bottomAngleVec.x, (float) sample.y + (float) bottomAngleVec.y, (float) sample.z + (float) bottomAngleVec.z).setColor(red, green, blue, alpha).setUv(f3, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
            this.vertex(vertexconsumer, matrix4f, (float) sample.x + (float) topAngleVec.x, (float) sample.y + (float) topAngleVec.y, (float) sample.z + (float) topAngleVec.z).setColor(red, green, blue, alpha).setUv(f3, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
            this.vertex(vertexconsumer, matrix4f, (float) draw1.x + (float) topAngleVec.x, (float) draw1.y + (float) topAngleVec.y, (float) draw1.z + (float) topAngleVec.z).setColor(red, green, blue, alpha).setUv(f2, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn).setNormal(n.x, n.y, n.z);
            samples++;
            drawFrom = sample;
        }
    }
    ...
    private VertexConsumer vertex(VertexConsumer consumer, Matrix4f matrix, float x, float y, float z) {
        org.joml.Vector4f vec = new org.joml.Vector4f(x, y, z, 1.0F).mul(matrix);
        return consumer.addVertex(vec.x, vec.y, vec.z);
    }
