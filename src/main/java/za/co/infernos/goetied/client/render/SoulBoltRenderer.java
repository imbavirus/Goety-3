...
import za.co.infernos.goetied.client.render.model.SoulBoltModel;
...
public class SoulBoltRenderer extends EntityRenderer<SoulBolt> {
   ...
   public void render(SoulBolt entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
      ...
      this.model.renderToBuffer(matrixStackIn, vertexconsumer, 15728640, OverlayTexture.NO_OVERLAY, 0x80FFFFFF);
      ...
      if (entityIn.hasTrail()) {
         ...
         renderTrail(entityIn, partialTicks, matrixStackIn, bufferIn, colorUtil.red, colorUtil.green, colorUtil.blue,
               0.6F, packedLightIn);
         matrixStackIn.popPose();
      }
    ...
    */
   private void renderTrail(SoulBolt entityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn,
      ...
      while (samples < sampleSize) {
         ...
         float f3 = f2 + 0.5F;
         vertexconsumer
               .addVertex(matrix4f, (float) draw1.x + (float) bottomAngleVec.x, (float) draw1.y + (float) bottomAngleVec.y,
                     (float) draw1.z + (float) bottomAngleVec.z)
               .setColor(red, green, blue, alpha).setUv(f2, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn)
         ...
         vertexconsumer
               ...
               .setColor(red, green, blue, alpha).setUv(f3, 1.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn)
               .setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
         vertexconsumer
               .addVertex(matrix4f, (float) sample.x + (float) topAngleVec.x, (float) sample.y + (float) topAngleVec.y,
                     (float) sample.z + (float) topAngleVec.z)
               .setColor(red, green, blue, alpha).setUv(f3, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn)
               .setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
         vertexconsumer
               .addVertex(matrix4f, (float) draw1.x + (float) topAngleVec.x, (float) draw1.y + (float) topAngleVec.y,
                     (float) draw1.z + (float) topAngleVec.z)
               .setColor(red, green, blue, alpha).setUv(f2, 0.0F).setOverlay(OverlayTexture.NO_OVERLAY).setLight(packedLightIn)
               .setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
         samples++;
         drawFrom = sample;
      }
