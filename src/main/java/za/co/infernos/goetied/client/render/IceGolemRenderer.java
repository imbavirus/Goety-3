...
import za.co.infernos.goetied.client.render.model.IceGolemModel;
...
public class IceGolemRenderer<T extends IceGolem> extends MobRenderer<T, IceGolemModel<T>> {
    private static final ResourceLocation TEXTURES = Goetied.location("textures/entity/servants/ice_golem.png");
    ...
    public static class IceGolemFrostLayer<T extends IceGolem, M extends IceGolemModel<T>> extends RenderLayer<T, M> {
        private static final ResourceLocation TEXTURES = Goetied.location("textures/entity/servants/ice_golem_spikes.png");
        ...
        public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T golem, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            if (golem.isUpgraded()) {
                coloredCutoutModelCopyLayerRender(this.getParentModel(), this.layerModel, TEXTURES, matrixStackIn, bufferIn, packedLightIn, golem, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, partialTicks, -1);
            }
        }
    }
}