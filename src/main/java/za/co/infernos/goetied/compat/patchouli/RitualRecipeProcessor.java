...
public class RitualRecipeProcessor {
     ...
     * 
     * @param recipeId The recipe ID (e.g., "goetied:convert_villager_to_ravaged")
        ...
        try {
            ...
            if (level != null) {
                try {
                    ...
                    Goetied.LOGGER.warn("Failed to load ritual recipe {} from recipe manager: {}", recipeId, e.getMessage());
                }
            }
            
            // If recipe not found, return empty map (Patchouli will handle gracefully)
            if (recipe == null) {
                Goetied.LOGGER.warn("Ritual recipe {} not found", recipeId);
            ...
            // In Goetied, pedestals are typically empty, but we can use a default
            for (int i = 1; i <= 12; i++) {
                replacements.put("pedestal" + i, "goetied:pedestal");
            }
            ...
            Goetied.LOGGER.error("Error processing ritual recipe {}: {}", recipeId, e.getMessage(), e);
            return java.util.Collections.emptyMap();
        }
    }
}
