    ...
    public Multimap<Holder<Attribute>, AttributeModifier> getAttributeModifiers(SlotContext slotContext,
        ...
        if (IronLoaded.IRON_SPELLBOOKS.isLoaded()){
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.RobesIronResist, false)) {
                map.put(IronAttributes.FIRE_MAGIC_RESIST, new AttributeModifier(Goetied.location("robes_iron_fire_resist"), 1.0F, AttributeModifier.Operation.ADD_VALUE));
                map.put(IronAttributes.NATURE_MAGIC_RESIST, new AttributeModifier(Goetied.location("robes_iron_nature_resist"), 0.5F, AttributeModifier.Operation.ADD_VALUE));
                map.put(IronAttributes.BLOOD_MAGIC_RESIST, new AttributeModifier(Goetied.location("robes_iron_blood_resist"), 0.75F, AttributeModifier.Operation.ADD_VALUE));
                map.put(IronAttributes.HOLY_MAGIC_RESIST, new AttributeModifier(Goetied.location("robes_iron_holy_resist"), -0.25F, AttributeModifier.Operation.ADD_VALUE));
            }
        }
        return map;
    }
