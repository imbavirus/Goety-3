...
public class RobeEvents {
    ...
    public static void LivingEffects(EntityTickEvent.Post event){
        ...
        if (livingEntity != null) {
            if (!livingEntity.level().isClientSide) {
                if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.CompatMinionHeal, false)) {
                ...
                if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.NecroSetDebuff, false) || za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.NamelessSetDebuff, false)) {
                    if (MobUtil.getOwner(livingEntity) != null){
                        if (!livingEntity.getType().is(EntityTypeTags.UNDEAD)
                            ...
                            boolean flag = false;
                            int amp = 0;
                            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.NecroSetDebuff, false)) {
                            ...
                            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.NamelessSetDebuff, false)) {
                            ...
                            if (flag) {
                                ...
                                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.SAPPED, 20, 2 + amp));
                            }
                        }
                    }
                }
    ...
    public static void HurtEvent(LivingIncomingDamageEvent event){
        ...
        if (CuriosFinder.hasFrostRobes(victim)){
            if (ModDamageSource.freezeAttacks(event.getSource()) || event.getSource().is(DamageTypeTags.IS_FREEZING)){
                float resistance = 1.0F - (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.FrostRobeResistance, 0) / 100.0F);
        ...
        if (CuriosFinder.hasNetherRobe(victim)){
            float resistance = 1.0F - (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.NetherRobeResistance, 0) / 100.0F);
            if (resistance <= 0.0F){
        ...
        if (CuriosFinder.hasCurio(victim, ModItems.GRAND_TURBAN.get())){
            if (victim instanceof Player player) {
                if (SEHelper.getSoulsAmount(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.ItemsRepairAmount, 0))) {
                    ...
                    if ((victim.level().random.nextBoolean() || victim.getHealth() <= victim.getMaxHealth() / 2) && irks < 16) {
                        ...
                        if (victim.level().addFreshEntity(irk)) {
                            SEHelper.decreaseSouls(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.ItemsRepairAmount, 0));
                        }
                    }
                }
            }
        ...
        if (CuriosFinder.hasUnholyHat(victim)){
            ...
            if (!event.getSource().is(DamageTypeTags.BYPASSES_INVULNERABILITY)){
                damage = Math.min(damage, za.co.infernos.goetied.utils.ConfigHelper.getFloat(AttributesConfig.ApostleDamageCap, 1.0F));
            }
        ...
        if(CuriosFinder.hasCurio(victim, ModItems.STORM_ROBE.get())){
            if (ModDamageSource.shockAttacks(event.getSource()) || event.getSource().is(DamageTypes.LIGHTNING_BOLT)){
                float resistance = 1.0F - (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.StormRobeResistance, 0) / 100.0F);
                event.setAmount(event.getAmount() * resistance);
            }
        ...
        if (CuriosFinder.hasWitchRobe(victim) || CuriosFinder.hasWarlockRobe(victim)){
            if (event.getSource().is(DamageTypeTags.WITCH_RESISTANT_TO)){
                if (!(LichdomHelper.isLich(victim) && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.LichMagicResist, false))){
                    float resistance = 1.0F - (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.WitchRobeResistance, 0) / 100.0F);
                    if (CuriosFinder.hasWarlockRobe(victim)){
                        resistance = 1.0F - (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.WarlockRobeResistance, 0) / 100.0F);
                    }
                    event.setAmount(event.getAmount() * resistance);
                }
            }
    ...
    public static void AttackEvent(LivingIncomingDamageEvent event){
        ...
        if (!event.getEntity().level().isClientSide) {
            ...
            if (event.getAmount() > 0.0F) {
                if (za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.VoidRobeTeleportChance, 0) > 0) {
                    if (CuriosFinder.hasVoidRobe(victim)) {
                        if (!victim.isInvulnerableTo(event.getSource()) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(victim)) {
                            float chance = za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.VoidRobeTeleportChance, 0) / 100.0F;
                            if (victim.getRandom().nextFloat() <= chance) {
                                double d0 = victim.getX() + (victim.getRandom().nextDouble() - 0.5D) * za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.VoidRobeTeleportDistance, 0);
                                double d1 = victim.getY();
                                double d2 = victim.getZ() + (victim.getRandom().nextDouble() - 0.5D) * za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.VoidRobeTeleportDistance, 0);
                                if (MobUtil.teleport(victim, d0, d1, d2)) {
                                    if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(ItemConfig.VoidRobeTeleportDamageCancel, false)) {
                                        event.setCanceled(true);
                                    }
                                }
                            }
                ...
                if (target instanceof Player) {
                    ...
                    if (CuriosFinder.neutralNecroSet(target) || CuriosFinder.neutralNamelessSet(target)) {
                        ...
                        if (target.level() instanceof ServerLevel serverLevel) {
                            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.HostileCryptUndead, false)) {
                    ...
                    if (CuriosFinder.neutralVoidSet(target)) {
                        ...
                        if (target.level() instanceof ServerLevel serverLevel) {
                            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.HostileTerminalEnder, false)) {
    ...
    public static void PotionApplicationEvents(MobEffectEvent.Applicable event){
        if (event.getEffectInstance().getEffect() == GoetiedEffects.FREEZING.get()){
        ...
        if (event.getEffectInstance().getEffect() == GoetiedEffects.BURN_HEX.get()){
        ...
        if (event.getEffectInstance().getEffect() == MobEffects.POISON
                || event.getEffectInstance().getEffect() == GoetiedEffects.ACID_VENOM.get()) {
        ...
        if (event.getEffectInstance().getEffect() == GoetiedEffects.VOID_TOUCHED.get()) {
