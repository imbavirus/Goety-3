...
import za.co.infernos.goetied.common.blocks.PithosBlock;
...
public class PithosBlockEntity extends RandomizableContainerBlockEntity {
    ...
    protected Component getDefaultName() {
        return Component.translatable("container.goetied.pithos");
    }
    ...
    private void updateBlockState(BlockState pState, boolean pOpen) {
        ...
        this.getLevel().setBlock(this.getBlockPos(), pState.setValue(PithosBlock.OPEN, pOpen), 3);
    }
    ...
    public void lock(){
        ...
        this.getLevel().setBlock(this.getBlockPos(), this.getBlockState().setValue(PithosBlock.TRIGGERED, Boolean.FALSE), 3);
    ...
    public void unlock(){
        ...
        if (this.getBlockState().getValue(PithosBlock.LOCKED)){
            this.getLevel().setBlock(this.getBlockPos(), this.getBlockState().setValue(PithosBlock.LOCKED, Boolean.FALSE), 3);
                    ...
                    for (int k = -range; k < range; ++k){
                        ...
                        if (blockState.is(ModBlocks.CRYPT_CHEST.get())){
                            if (blockState.hasProperty(CryptChestBlock.LOCKED) && blockState.getValue(CryptChestBlock.LOCKED)) {
                                this.getLevel().setBlock(blockPos, blockState.setValue(CryptChestBlock.LOCKED, false), 3);
