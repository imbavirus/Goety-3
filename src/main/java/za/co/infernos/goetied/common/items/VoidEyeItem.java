...
import za.co.infernos.goetied.common.entities.projectiles.EyeItemEntity;
...
public class VoidEyeItem extends Item {
   ...
   public InteractionResultHolder<ItemStack> use(Level p_41184_, Player p_41185_, InteractionHand p_41186_) {
      ...
      if (p_41184_ instanceof ServerLevel serverlevel) {
          BlockPos blockpos = serverlevel.findNearestMapStructure(ModStructureTags.FINAL_TERMINAL, p_41185_.blockPosition(), 100, false);
         if (blockpos != null) {
            EyeItemEntity eyeItem = new EyeItemEntity(ModEntityType.VOID_EYE.get(), p_41184_, p_41185_.getX(), p_41185_.getY(0.5D), p_41185_.getZ());
            eyeItem.setItem(itemstack);
            eyeItem.signalTo(blockpos);
            eyeItem.setSurviveAfterDeath(false);
            eyeItem.setParticle(ParticleTypes.PORTAL);
