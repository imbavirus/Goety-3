    ...
    public void hurtMobs(LivingEntity living){
        if (this.getTrueOwner() != null) {
            if (CuriosFinder.hasUnholySet(this.getTrueOwner())) {
                if (living.hurt(ModDamageSource.hellfire(this, this.getTrueOwner()), (float)za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ApostleMagicDamage, 20.0D) / 1.5F)){
                    living.addEffect(new MobEffectInstance(GoetiedEffects.BURN_HEX, 1200));
                }
            } else {
