...
import za.co.infernos.goetied.client.particles.WindBlowParticle;
import za.co.infernos.goetied.common.blocks.VoidShrineBlock;
                ...
                if (!this.getBlockState().getValue(VoidShrineBlock.TRIGGERED)) {
                    ...
                    if (this.coolTick < 20) {
                        for (Player player : serverLevel.getEntitiesOfClass(Player.class, new AABB(this.getBlockPos()).inflate(64.0F), EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
                            player.addEffect(new MobEffectInstance(GoetiedEffects.IMPAIRED, 5, 0, false, false));
                        }
                    }
