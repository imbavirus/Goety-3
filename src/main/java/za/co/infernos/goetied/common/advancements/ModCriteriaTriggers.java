package za.co.infernos.goetied.common.advancements;
...
public class ModCriteriaTriggers {
    ...
    public static void register(RegisterEvent event) {
        if (event.getRegistryKey() == Registries.TRIGGER_TYPE) {
            SERVANT_KILLED_ENTITY_INTERNAL = CriteriaTriggers.register(Goetied.location("servant_killed_entity").toString(), new KilledTrigger());
            SERVANT_RAID_VICTORY_INTERNAL = CriteriaTriggers.register(Goetied.location("servant_raid_victory").toString(), new PlayerTrigger());
        }
    }
