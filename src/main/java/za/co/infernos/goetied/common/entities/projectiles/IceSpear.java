...
public class IceSpear extends IceSpike {
    ...
    protected void onHitEntity(EntityHitResult p_37626_) {
        if (!this.level().isClientSide) {
            float baseDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.IceSpikeDamage, 1.0F) * WandUtil.damageMultiply();
            ...
            if (flag && entity instanceof LivingEntity livingEntity) {
                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, MathHelper.secondsToTicks(3 + livingEntity.getRandom().nextInt(2))));
