...
import za.co.infernos.goetied.api.blocks.entities.IWindPowered;
...
public class SquallGolem extends AbstractGolemServant implements IWindPowered {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemHealth, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemArmor, 20.0D))
                .add(Attributes.MOVEMENT_SPEED, 0.3D)
                .add(Attributes.KNOCKBACK_RESISTANCE, 1.0D)
                .add(Attributes.ATTACK_KNOCKBACK, 1.25D)
                .add(Attributes.STEP_HEIGHT, 1.0D)
                .add(Attributes.ATTACK_DAMAGE, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemDamage, 20.0D))
                .add(Attributes.FOLLOW_RANGE, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemFollowRange, 20.0D));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemArmor, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemDamage, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.FOLLOW_RANGE), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemFollowRange, 20.0D));
    }
    ...
    public void updateMoveMode(Player player) {
        if (!this.isStaying() && !this.isGuardingArea()){
            ...
            player.displayClientMessage(Component.translatable("info.goetied.servant.staying", this.getDisplayName()), true);
        } else if (!this.isGuardingArea()){
            this.setBoundPos(this.blockPosition());
            this.setStaying(false);
            player.displayClientMessage(Component.translatable("info.goetied.servant.guard", this.getDisplayName()), true);
        } else {
            this.setBoundPos(null);
            this.setStaying(false);
            player.displayClientMessage(Component.translatable("info.goetied.servant.follow", this.getDisplayName()), true);
        }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pReason, @Nullable SpawnGroupData pSpawnData) {
        if (pReason == MobSpawnType.STRUCTURE) {
            ...
            this.setBoundPos(this.blockPosition());
            this.setHealth((float)za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.SquallGolemHealth, 20.0D));
    ...
    class MeleeGoal extends Goal {
        ...
        public void hurtTarget(Entity target) {
            ...
            boolean flag = target.hurt(SquallGolem.this.getServantAttack(), f);
            if (flag) {
