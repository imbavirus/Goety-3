...
public class WeakeningSpell extends Spell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.WeakeningCost, 0);
    }

    @Override
    public int defaultCastDuration() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.WeakeningDuration, 0);
    }
    ...
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.WeakeningCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat){
        ...
        for (LivingEntity livingEntity : worldIn.getEntitiesOfClass(LivingEntity.class, caster.getBoundingBox().inflate(radius))){
            if (!MobUtil.areAllies(caster, livingEntity)) {
                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.SAPPED, MathHelper.secondsToTicks(duration), potency + 4));
