...
import za.co.infernos.goetied.client.particles.DustCloudParticleOption;
import za.co.infernos.goetied.client.particles.ModParticleTypes;
import za.co.infernos.goetied.client.render.HellblastTextures;
...
public class HellBlast extends WaterHurtingProjectile {
    private static final EntityDataAccessor<Integer> DATA_TYPE_ID = SynchedEntityData.defineId(HellBlast.class,
            EntityDataSerializers.INT);
    public static final EntityDataAccessor<Integer> DATA_FIERY = SynchedEntityData.defineId(HellBlast.class,
            EntityDataSerializers.INT);
    public static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(HellBlast.class,
            EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DATA_DAMAGE = SynchedEntityData.defineId(HellBlast.class,
            EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> DATA_EXTRA_DAMAGE = SynchedEntityData.defineId(HellBlast.class,
            EntityDataSerializers.FLOAT);
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        ...
        if (!this.level().isClientSide) {
            ...
            if (entity1 instanceof Player) {
                damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.LavaballDamage, 1.0F) * WandUtil.damageMultiply();
            }
                ...
                public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z,
                    ...
                    if (damageSource.getDirectEntity() instanceof HellBlast hellBlast) {
                        if (hellBlast.getFiery() > 0) {
                            entity.igniteForSeconds(5 * hellBlast.getFiery());
                        }
                    }
                }
            };
