                    ...
                    for (Entity entity : targets) {
                        if ((this.owner != null && CuriosFinder.hasUnholySet(this.owner))) {
                            if (entity instanceof LivingEntity livingEntity) {
                                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.BURN_HEX, 1200));
                            }
                            entity.hurt(ModDamageSource.hellfire(this, this.owner),
                                    (float)za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ApostleMagicDamage, 20.0D) + this.getExtraDamage());
                        } else {
                            if (this.owner != null) {
                                float damage = 5.0F;
