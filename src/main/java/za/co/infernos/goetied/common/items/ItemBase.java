    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip,
        ...
        if (stack.getItem() instanceof ItemBase) {
            if (stack.is(ModItems.OMINOUS_SADDLE.get())) {
                tooltip.add(Component.translatable("info.goetied.ominous_saddle").withStyle(main));
            }
            if (stack.is(ModItems.VOID_KEY.get())) {
                tooltip.add(Component.translatable("info.goetied.void_key").withStyle(main));
            }
            if (stack.is(ModItems.VOID_SHARD.get())) {
                tooltip.add(Component.translatable("info.goetied.void_shard").withStyle(main));
            }
        }
    }
}