...
import za.co.infernos.goetied.common.entities.neutral.Wartling;
import za.co.infernos.goetied.common.entities.projectiles.BerserkFungus;
...
public class Warlock extends Cultist implements RangedAttackMob {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WarlockHealth, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WarlockArmor, 20.0D))
                .add(Attributes.MOVEMENT_SPEED, 0.25D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WarlockHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WarlockArmor, 20.0D));
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor p_37856_, DifficultyInstance p_37857_, MobSpawnType p_37858_, @Nullable SpawnGroupData p_37859_) {
        if (p_37858_ == MobSpawnType.EVENT) {
            if (p_37856_.getLevel().random.nextFloat() <= 0.25F) {
                ...
                donkey.finalizeSpawn(p_37856_, p_37856_.getLevel().getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.EVENT, null);
                donkey.setAge(0);
    ...
    private void summonWartlings(Wartling wartling){
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            // getCurativeItems removed in 1.21, commenting out
            // this.getActiveEffects().stream().filter(mobEffect -> mobEffect.getEffect().value().getCategory() == MobEffectCategory.HARMFUL && !mobEffect.getEffect().getCurativeItems().isEmpty()).findFirst().ifPresent(effect -> {
