...
public class TelekinesisSpell extends EverChargeSpell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.TelekinesisCost, 0);
    }

    @Override
    public int defaultCastUp() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.TelekinesisChargeUp, 0);
    }
        ...
        if (target != null) {
            ...
            if ((caster.getBoundingBox().inflate(0.5D).getSize() * potency) >= target.getBoundingBox().getSize()) {
                if (this.victim instanceof LivingEntity livingTarget){
                    if (livingTarget.getMaxHealth() >= za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.TelekinesisMaxHealth, 0)
                            || MobUtil.hasEntityTypesConfig(za.co.infernos.goetied.utils.ConfigHelper.getList(SpellConfig.TelekinesisBlackList, java.util.Collections.emptyList()), livingTarget.getType())){
                        flag = false;
                    }
                }
            }
