...
public class BoneheadHammerItem extends HammerItem {
    ...
    public void smash(ItemStack pStack, LivingEntity pTarget, Player player) {
        ...
        player.level().playSound((Player) null, player.getX(), player.getY(), player.getZ(),
                ModSounds.BONEHEAD_HAMMER_HIT.get(), player.getSoundSource(), 1.0F, 1.0F);
    ...
    public void attackMobs(LivingEntity pTarget, Player pPlayer, ItemStack pStack) {
        ...
        for (LivingEntity livingentity : pPlayer.level().getEntitiesOfClass(LivingEntity.class,
                pTarget.getBoundingBox().inflate(area * 2.0D, 1.0D, area * 2.0D))) {
            if (livingentity != pPlayer && livingentity != pTarget && !MobUtil.areAllies(pPlayer, livingentity)
                    && (!(livingentity instanceof ArmorStand) || !((ArmorStand) livingentity).isMarker())
                    && livingentity != pPlayer.getVehicle()) {
                Vec3 vec3 = livingentity.position().subtract(pTarget.position());
                vec3 = vec3.normalize();
                livingentity.level().playSound((Player) null, livingentity.getX(), livingentity.getY(),
                        livingentity.getZ(), ModSounds.GRAVITY.get(), pPlayer.getSoundSource(), 1.0F, 1.0F);
                MobUtil.pull(livingentity, vec3.x, vec3.y, vec3.z);
                if (livingentity.level() instanceof ServerLevel serverLevel) {
                    serverLevel.sendParticles(new TeleportInShockwaveParticleOption(), livingentity.getX(),
                            livingentity.getY() + 0.25F, livingentity.getZ(), 0, 0, 0, 0, 0.5F);
                }
            }
        }

        pPlayer.level().playSound((Player) null, pPlayer.getX(), pPlayer.getY(), pPlayer.getZ(),
                ModSounds.BONEHEAD_HAMMER_IMPACT.get(), pPlayer.getSoundSource(), 1.0F, 1.0F);
    }
