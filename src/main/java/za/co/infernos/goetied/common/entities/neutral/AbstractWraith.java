...
import za.co.infernos.goetied.common.entities.ai.FloatSwimGoal;
import za.co.infernos.goetied.common.entities.ai.SummonTargetGoal;
...
import za.co.infernos.goetied.utils.SoundUtil;
...
public class AbstractWraith extends Summoned {
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithHealth, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithArmor, 20.0D))
                .add(Attributes.FOLLOW_RANGE, 16.0D)
                .add(Attributes.MOVEMENT_SPEED, 0.25F)
                .add(Attributes.STEP_HEIGHT, 1.0F)
                .add(Attributes.ATTACK_DAMAGE, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithDamage, 20.0D));
    }

    public void setConfigurableAttributes() {
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithArmor, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.WraithDamage, 20.0D));
    }
    ...
    public int getSummonLimit(LivingEntity owner) {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.WraithLimit, 6);
    }
    ...
    protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource damageSource, boolean wasRecentlyHit) {
        ...
        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Wraith dropping loot - Table key: {}", lootTableKey.location());
        
        // Call super first
        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Calling super.dropCustomDeathLoot()...");
        super.dropCustomDeathLoot(serverLevel, damageSource, wasRecentlyHit);
        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] super.dropCustomDeathLoot() completed");
        
        // Now manually process the loot table with extensive logging
        if (serverLevel.getServer() != null) {
            try {
                // Step 1: Retrieve loot table
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Retrieving loot table from registry...");
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Loot table key: {} (namespace: {}, path: {})", 
                    lootTableKey.location(), lootTableKey.location().getNamespace(), lootTableKey.location().getPath());
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Expected resource path: data/{}/loot_tables/{}.json", 
                    lootTableKey.location().getNamespace(), lootTableKey.location().getPath());
                LootTable lootTable = serverLevel.getServer().reloadableRegistries().getLootTable(lootTableKey);
                
                if (lootTable == null) {
                    za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Loot table is NULL!");
                    return;
                }
                
                ResourceLocation tableId = lootTable.getLootTableId();
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Loot table retrieved successfully. Table ID: {}", tableId);
                
                // Check if it's the empty loot table
                if (lootTable == LootTable.EMPTY) {
                    za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Loot table is EMPTY! This means the table wasn't found or loaded.");
                    za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Expected table: {}, but got EMPTY table", lootTableKey.location());
                    za.co.infernos.goetied.Goetied.LOGGER.warn("[LOOT DEBUG] Since loot table loading is broken, using fallback manual drop. The loot table file exists and is valid JSON, but Minecraft isn't loading it.");
                    // Skip the rest of the loot table processing and go straight to fallback
                    if (za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.isBound()) {
                        int count = 1 + serverLevel.random.nextInt(2); // 1-2 ectoplasm
                        this.spawnAtLocation(new net.minecraft.world.item.ItemStack(za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.get(), count));
                        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Manually dropped {} ectoplasm as fallback (loot table system not working)", count);
                    }
                    return; // Exit early, skip all the loot table processing
                } else {
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Loot table is not empty. Comparing with EMPTY: {}", lootTable == LootTable.EMPTY);
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Loot table EMPTY ID: {}", LootTable.EMPTY.getLootTableId());
                }
                ...
                try {
                    ...
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Checking for resource at PLURAL path: {}", resourceLocationPlural);
                    var resourcePlural = resourceManager.getResource(resourceLocationPlural);
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Plural path exists: {}", resourcePlural.isPresent());
                    
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Checking for resource at SINGULAR path: {}", resourceLocationSingular);
                    var resourceSingular = resourceManager.getResource(resourceLocationSingular);
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Singular path exists: {}", resourceSingular.isPresent());
                    ...
                    if (resource.isPresent()) {
                        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Resource file EXISTS at: {} (using {})", resourceLocation, resourcePlural.isPresent() ? "PLURAL" : "SINGULAR");
                        // Try to read the file content and manually parse it
                        try (var inputStream = resource.get().open()) {
                            String content = new String(inputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8);
                            za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Resource file content (first 200 chars): {}", 
                            ...
                            try {
                                ...
                                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] JSON parsed successfully. Type: {}, Pools: {}", 
                                    jsonObject.get("type"), jsonObject.get("pools") != null ? jsonObject.getAsJsonArray("pools").size() : "null");
                            } catch (Exception parseEx) {
                                za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] JSON parsing error: ", parseEx);
                            }
                        }
                    } else {
                        za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Resource file NOT FOUND at: {}", resourceLocation);
                    }
                } catch (Exception e) {
                    za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Error checking resource file: ", e);
                }
                ...
                var ectoplasmKey = net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.ITEM, Goetied.location("ectoplasm"));
                boolean itemExists = itemRegistry.containsKey(ectoplasmKey);
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Ectoplasm item exists in registry: {}", itemExists);
                
                if (itemExists) {
                    var ectoplasmItem = itemRegistry.get(ectoplasmKey);
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Ectoplasm item: {} (bound: {})", 
                        ectoplasmItem != null ? ectoplasmItem.getDescriptionId() : "null",
                        za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.isBound());
                }
                
                // Step 3: Build LootParams
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Building LootParams...");
                ...
                if (this.lastHurtByPlayerTime > 0 && this.lastHurtByPlayer != null) {
                    ...
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Added player context - Luck: {}", this.lastHurtByPlayer.getLuck());
                } else {
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] No player context (lastHurtByPlayerTime: {}, lastHurtByPlayer: {})", 
                ...
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] LootParams created. Context type: ENTITY");
                
                // Step 4: Try method 1 - getRandomItems without seed (like Inferno)
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 1: Calling getRandomItems(lootParams) without seed...");
                java.util.List<net.minecraft.world.item.ItemStack> lootItems1 = lootTable.getRandomItems(lootParams);
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 1 result: {} items generated", lootItems1.size());
                if (!lootItems1.isEmpty()) {
                    lootItems1.forEach(item -> {
                        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 1 item: {} x{}", item.getItem().getDescriptionId(), item.getCount());
                        this.spawnAtLocation(item);
                    });
                }
                
                // Step 5: Try method 2 - getRandomItems with seed and callback (like CryptSlime)
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 2: Calling getRandomItems(lootParams, seed, callback) with seed {}...", this.getLootTableSeed());
                ...
                lootTable.getRandomItems(lootParams, this.getLootTableSeed(), (itemStack) -> {
                    ...
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 2 callback invoked: {} x{}", itemStack.getItem().getDescriptionId(), itemStack.getCount());
                    this.spawnAtLocation(itemStack);
                });
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Method 2 result: {} items generated (callback count)", lootItems2.size());
                
                // Step 6: Summary
                int totalItems = lootItems1.size() + lootItems2.size();
                za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Total items generated: {} (Method 1: {}, Method 2: {})", totalItems, lootItems1.size(), lootItems2.size());
                
                // Fallback if still empty
                if (totalItems == 0) {
                    za.co.infernos.goetied.Goetied.LOGGER.warn("[LOOT DEBUG] Both methods generated 0 items! Using fallback manual drop.");
                    if (za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.isBound()) {
                        int count = 1 + serverLevel.random.nextInt(2); // 1-2 ectoplasm
                        this.spawnAtLocation(new net.minecraft.world.item.ItemStack(za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.get(), count));
                        za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Manually dropped {} ectoplasm as fallback", count);
                    } else {
                        za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] ECTOPLASM item is not bound for fallback!");
                    }
                }
            } catch (Exception e) {
                za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Exception during loot processing: ", e);
                // Fallback on error
                if (za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.isBound()) {
                    int count = 1 + serverLevel.random.nextInt(2);
                    this.spawnAtLocation(new net.minecraft.world.item.ItemStack(za.co.infernos.goetied.common.items.ModItems.ECTOPLASM.get(), count));
                    za.co.infernos.goetied.Goetied.LOGGER.info("[LOOT DEBUG] Manually dropped {} ectoplasm as error fallback", count);
                }
            }
        } else {
            za.co.infernos.goetied.Goetied.LOGGER.error("[LOOT DEBUG] Server is null!");
        }
    }
        ...
        if (!this.level().isClientSide) {
            ...
            if (this.getTarget() != null && !this.isPostTeleporting()) {
                ...
                if (this.getSensing().hasLineOfSight(this.getTarget())) {
                    ...
                    if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.WraithAggressiveTeleport, true)) {
                        if (this.canTeleport()) {
                            this.getNavigation().stop();
                            this.setIsTeleporting(true);
                        }
