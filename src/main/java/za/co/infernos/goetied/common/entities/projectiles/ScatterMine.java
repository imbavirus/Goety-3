...
import za.co.infernos.goetied.client.particles.PulsatingCircleParticleOption;
...
public class ScatterMine extends Entity implements ISpellEntity {
    ...
    public void explodeDamage(LivingEntity livingEntity) {
        if (!this.level().isClientSide) {
            ...
            float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.ScatterMineDamage, 1.0F);
            if (!this.isSpell()) {
