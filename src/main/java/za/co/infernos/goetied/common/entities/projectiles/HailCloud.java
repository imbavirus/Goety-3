...
public class HailCloud extends AbstractSpellCloud{
    ...
    public void hurtEntities(LivingEntity livingEntity){
        if (livingEntity != null) {
            float baseDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.HailDamage, 1.0F) * WandUtil.damageMultiply();
            baseDamage += this.getExtraDamage();
            if (livingEntity.hurt(ModDamageSource.frostBreath(this, this.getOwner()), baseDamage)) {
                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, MathHelper.secondsToTicks(5)));
            }
        }
    }
}
