...
public class BrewEffectCloud extends Entity {
    private static final EntityDataAccessor<Float> DATA_RADIUS = SynchedEntityData.defineId(BrewEffectCloud.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> DATA_COLOR = SynchedEntityData.defineId(BrewEffectCloud.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> DATA_WAITING = SynchedEntityData.defineId(BrewEffectCloud.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<ParticleOptions> DATA_PARTICLE = SynchedEntityData.defineId(BrewEffectCloud.class, EntityDataSerializers.PARTICLE);
    ...
    public BrewEffectCloud(Level p_19707_, double p_19708_, double p_19709_, double p_19710_) {
        this(ModEntityType.BREW_EFFECT_CLOUD.get(), p_19707_);
                                    ...
                                    for(MobEffectInstance mobeffectinstance1 : this.effects) {
                                        if (mobeffectinstance1.getEffect().value().isInstantenous()) {
                                            ...
                                            livingentity.addEffect(new MobEffectInstance(mobeffectinstance1.getEffect(), mobeffectinstance1.mapDuration((p_267926_) -> {
                                                return p_267926_ / 4;
                                            }), mobeffectinstance1.getAmplifier(), mobeffectinstance1.isAmbient(), mobeffectinstance1.isVisible()));
                                        }
                                    }

                                    for(BrewEffectInstance mobeffectinstance1 : this.brewEffects) {
                                        if (mobeffectinstance1.getEffect().canLinger()) {
    ...
    protected void readAdditionalSaveData(CompoundTag p_19727_) {
        ...
        if (p_19727_.contains("BrewEffects", 9)) {
            ListTag listtag = p_19727_.getList("BrewEffects", 10);
            this.brewEffects.clear();

            for(int i = 0; i < listtag.size(); ++i) {
                BrewEffectInstance brewInstances = BrewEffectInstance.load(listtag.getCompound(i));
                if (brewInstances != null) {
    ...
    protected void addAdditionalSaveData(CompoundTag p_19737_) {
        ...
        if (!this.brewEffects.isEmpty()) {
            ...
            p_19737_.put("BrewEffects", listtag);
        }
