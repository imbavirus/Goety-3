package za.co.infernos.goetied.common.entities.ally.undead.bound;

import za.co.infernos.goetied.api.entities.IBreathing;
...
import za.co.infernos.goetied.common.entities.ai.BreathingAttackGoal;
...
import za.co.infernos.goetied.common.magic.spells.frost.HailSpell;
import za.co.infernos.goetied.common.magic.spells.frost.IceChunkSpell;
...
public class BoundCryologer extends AbstractBoundIllager implements IBreathing {
    private static final EntityDataAccessor<Byte> IS_CASTING_SPELL = SynchedEntityData.defineId(BoundCryologer.class, EntityDataSerializers.BYTE);
    private static final EntityDataAccessor<Integer> ANIM_STATE = SynchedEntityData.defineId(BoundCryologer.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> BREATHING = SynchedEntityData.defineId(BoundCryologer.class, EntityDataSerializers.BOOLEAN);
    ...
    public BoundCryologer(EntityType<? extends AbstractBoundIllager> p_i48551_1_, Level p_i48551_2_) {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(4, new AvoidTargetGoal<>(this, LivingEntity.class, 8.0F, 0.6D, 1.0D){
            ...
            public boolean canUse() {
                return super.canUse() && BoundCryologer.this.getCurrentAnimation() < 2;
            }
        });
    }
    ...
    public static AttributeSupplier.Builder setCustomAttributes(){
        return Mob.createMobAttributes()
                ...
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerHealth, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerArmor, 20.0D))
                .add(Attributes.MOVEMENT_SPEED, 0.5D)
                .add(Attributes.FLYING_SPEED, 0.15D)
                .add(Attributes.ATTACK_DAMAGE, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerDamage, 20.0D));
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerArmor, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.CryologerDamage, 20.0D));
    }
    ...
    public void doBreathing(Entity target) {
        ...
        if (target.hurt(ModDamageSource.frostBreath(this, this), damage)) {
            if (target instanceof LivingEntity living) {
                living.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, MathHelper.secondsToTicks(1)));
            }
        }
    }

    class BreathGoal extends BreathingAttackGoal<BoundCryologer> {
        protected int nextAttackTickCount;
        protected int breathTime;

        public BreathGoal() {
            super(BoundCryologer.this, 8, 20, 1.0F);
        }
        ...
        public boolean canUse() {
            return super.canUse()
                    && !BoundCryologer.this.isCastingSpell()
                    && BoundCryologer.this.tickCount >= this.nextAttackTickCount
                    && noWall();
        }
        ...
        public void start() {
            ...
            this.nextAttackTickCount = BoundCryologer.this.tickCount + 100;
            this.breathTime = 10;
        }
    ...
    class CastingSpellGoal extends Goal {
        ...
        public boolean canUse() {
            return BoundCryologer.this.getSpellCastingTime() > 0;
        }

        public void start() {
            super.start();
            BoundCryologer.this.navigation.stop();
        }

        public void stop() {
            super.stop();
            BoundCryologer.this.setIsCastingSpell(0);
            BoundCryologer.this.setAnimationState("idle");
        }

        public void tick() {
            if (BoundCryologer.this.getTarget() != null) {
                BoundCryologer.this.getLookControl().setLookAt(BoundCryologer.this.getTarget(), (float) BoundCryologer.this.getMaxHeadYRot(), (float) BoundCryologer.this.getMaxHeadXRot());
            }
    ...
    protected abstract class CryologerUseSpellGoal extends Goal {
        ...
        public boolean canContinueToUse() {
            LivingEntity livingentity = BoundCryologer.this.getTarget();
            return livingentity != null && livingentity.isAlive() && BoundCryologer.this.hasLineOfSight(livingentity) && this.attackWarmupDelay > 0;
        }

        public void start() {
            this.attackWarmupDelay = this.adjustedTickDelay(this.getCastWarmupTime());
            BoundCryologer.this.castingTime = this.getCastingTime();
            this.nextAttackTickCount = BoundCryologer.this.tickCount + this.getCastingInterval();
            SoundEvent soundevent = this.getSpellPrepareSound();
            if (soundevent != null) {
                BoundCryologer.this.playSound(soundevent, 1.0F, 1.0F);
            }
        }
        ...
        public void tick() {
            ...
            if (this.attackWarmupDelay == 0) {
                ...
                BoundCryologer.this.playSound(BoundCryologer.this.getCastingSoundEvent(), 1.0F, 1.0F);
            }
    ...
    class HailSpellGoal extends CryologerUseSpellGoal {
        ...
        public void start() {
            super.start();
            BoundCryologer.this.setAnimationState("cloud");
        }

        @Override
        protected void performSpellCasting() {
            if (BoundCryologer.this.getTarget() != null){
                new HailSpell().mobSpellResult(BoundCryologer.this, ItemStack.EMPTY);
            }
        }
    ...
    class WallSpellGoal extends CryologerUseSpellGoal {
        ...
        public void start() {
            super.start();
            BoundCryologer.this.setAnimationState("wall");
        }

        @Override
        protected void performSpellCasting() {
            if (BoundCryologer.this.getTarget() != null){
                LivingEntity target = BoundCryologer.this.getTarget();
                int random = BoundCryologer.this.random.nextInt(3);
                if (random == 0) {
                    int[] rowToRemove = Util.getRandom(WandUtil.CONFIG_1_ROWS, BoundCryologer.this.getRandom());
                    ...
                    WandUtil.summonLesserSquareTrap(BoundCryologer.this, target.blockPosition(), ModEntityType.GLACIAL_WALL.get(), rowToRemove, 1);
                } else if (random == 1){
                    WandUtil.summonWallTrap(BoundCryologer.this, target, ModEntityType.GLACIAL_WALL.get(), 3, 1);
                } else {
                    WandUtil.summonRandomPillarsTrap(BoundCryologer.this, target, ModEntityType.GLACIAL_WALL.get(), 6, 1);
                }
            }
        }
    ...
    class ChunkSpellGoal extends CryologerUseSpellGoal {
        ...
        public boolean canUse() {
            return super.canUse()
                    ...
                    && za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MobsConfig.CryologerIceChunk, false);
        }

        public void start() {
            super.start();
            BoundCryologer.this.setAnimationState("chunk");
            if (BoundCryologer.this.getTarget() != null){
                new IceChunkSpell().mobSpellResult(BoundCryologer.this, STAFF);
            }
        }
