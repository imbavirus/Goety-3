...
import za.co.infernos.goetied.client.particles.GatherFrostParticle;
...
import za.co.infernos.goetied.common.entities.neutral.GlacialWall;
...
public class IceChunk extends SpellEntity {
    ...
    public void damageTargets(LivingEntity livingEntity){
        float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.IceChunkDamage, 1.0F) * WandUtil.damageMultiply();
        damage += this.getExtraDamage();
        if (livingEntity != null) {
            if (livingEntity.hurt(ModDamageSource.indirectFreeze(this, this.getOwner()), damage)) {
                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.STUNNED, MathHelper.secondsToTicks(2)));
            }
        }
    }
