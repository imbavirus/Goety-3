...
public class PikerServant extends AbstractIllagerServant {
    protected static final EntityDataAccessor<Byte> DATA_FLAGS_ID = SynchedEntityData.defineId(PikerServant.class,
    ...
    public PikerServant(EntityType<? extends Owned> type, Level worldIn) {
        super(type, worldIn);
    }

    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new PikerServant.MeleeGoal());
        this.goalSelector.addGoal(4, new PikerServant.PikerAttackGoal());
    }

    public static AttributeSupplier.Builder setCustomAttributes() {
        return Mob.createMobAttributes()
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerHealth, 20.0D))
                .add(Attributes.FOLLOW_RANGE, 32.0D)
                .add(Attributes.MOVEMENT_SPEED, 0.35D)
                .add(Attributes.ATTACK_DAMAGE, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerDamage, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerArmor, 20.0D));
    }

    public void setConfigurableAttributes() {
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ATTACK_DAMAGE), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerDamage, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.PikerArmor, 20.0D));
    }
    ...
    class PikerAttackGoal extends MeleeAttackGoal {
        ...
        public PikerAttackGoal() {
            super(PikerServant.this, SPEED, true);
        }

        @Override
        public boolean canUse() {
            return PikerServant.this.getTarget() != null && PikerServant.this.getTarget().isAlive();
        }

        @Override
        public void start() {
            PikerServant.this.setAggressive(true);
            this.delayCounter = 0;
        }

        @Override
        public void tick() {
            LivingEntity livingentity = PikerServant.this.getTarget();
            if (livingentity == null) {
                return;
            }

            PikerServant.this.lookControl.setLookAt(livingentity, 30.0F, 30.0F);
            double d0 = PikerServant.this.distanceToSqr(livingentity.getX(), livingentity.getY(), livingentity.getZ());

            if (--this.delayCounter <= 0 && !PikerServant.this.targetClose(livingentity, d0)) {
                this.delayCounter = 10;
                PikerServant.this.getNavigation().moveTo(livingentity, SPEED);
            }

            this.checkAndPerformAttack(livingentity, PikerServant.this.distanceToSqr(livingentity.getX(),
        ...
        @Override
        public void stop() {
            PikerServant.this.getNavigation().stop();
            if (PikerServant.this.getTarget() == null) {
                PikerServant.this.setAggressive(false);
            }
        }
    }
    ...
    class MeleeGoal extends Goal {
        ...
        @Override
        public void tick() {
            if (PikerServant.this.getTarget() != null && PikerServant.this.getTarget().isAlive()) {
                LivingEntity livingentity = PikerServant.this.getTarget();
                double d0 = PikerServant.this.distanceToSqr(livingentity.getX(), livingentity.getY(),
                        livingentity.getZ());
                MobUtil.instaLook(PikerServant.this, livingentity);
                if (PikerServant.this.attackTick == 1) {
                    PikerServant.this.playSound(ModSounds.PIKER_SWING.get(), PikerServant.this.getSoundVolume(),
                            PikerServant.this.getVoicePitch());
                }
                if (PikerServant.this.attackTick == 8) {
                    if (PikerServant.this.targetClose(livingentity, d0)) {
                            ...
                            for (Entity entity : getTargets(PikerServant.this.level(), PikerServant.this, 3)) {
                                if (entity instanceof LivingEntity living && PikerServant.this.hasLineOfSight(living)) {
                                    if (!MobUtil.areAllies(PikerServant.this, living) && living != livingentity
                                            ...
                                            && PikerServant.this.canAttack(livingentity)) {
