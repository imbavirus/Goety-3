...
public class FrostNovaSpell extends Spell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FrostNovaCost, 0);
    }

    @Override
    public int defaultCastDuration() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FrostNovaDuration, 0);
    }
    ...
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FrostNovaCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.FrostNovaDamage, 1.0F) * WandUtil.damageMultiply();
        float maxDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.FrostNovaMaxDamage, 1.0F) * WandUtil.damageMultiply();
        ...
        new SpellExplosion(worldIn, caster, ModDamageSource.directFreeze(caster), spellTarget.blockPosition(), (float) radius, trueDamage){
            ...
            public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z, double seen, float actualDamage) {
                if (target instanceof LivingEntity target1 && !MobUtil.areAllies(caster, target1) && EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(target1)){
                    ...
                    target1.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, MathHelper.secondsToTicks(5) * finalDuration, finalAmp));
                }
            }
        };
