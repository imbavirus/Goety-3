...
public class RecallFocus extends Item {
    ...
    public static void addRecallText(ItemStack stack, List<Component> tooltip) {
        ...
        if (blockPos != null && dim.isPresent()) {
            ...
            tooltip.add(Component.translatable("info.goetied.focus.Position", blockPos.getX(), blockPos.getY(), blockPos.getZ()).withStyle(ChatFormatting.GRAY));
            tooltip.add(Component.translatable("info.goetied.focus.PosDim", loc.toString()).withStyle(ChatFormatting.DARK_GRAY));
        }
    }
