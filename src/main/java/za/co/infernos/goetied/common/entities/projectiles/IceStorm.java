...
public class IceStorm extends SpellHurtingProjectile {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide){
            ...
            float baseDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.IceStormDamage, 1.0F) * WandUtil.damageMultiply();
                    ...
                    if (owner != null) {
                        if (!MobUtil.areAllies(owner, livingEntity) && livingEntity != owner) {
                            if (livingEntity.hurt(ModDamageSource.indirectFreeze(this, owner), baseDamage + this.getExtraDamage())) {
                                livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, MathHelper.secondsToTicks(1 + this.getDuration())));
                            }
                        }
                    } else {
