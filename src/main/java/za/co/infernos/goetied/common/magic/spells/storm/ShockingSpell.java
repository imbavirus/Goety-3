...
public class ShockingSpell extends EverChargeSpell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ShockingCost, 0);
    }

    @Override
    public int defaultCastUp() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ShockingChargeUp, 0);
    }

    @Override
    public int shotsNumber() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ShockingDuration, 0);
    }

    @Override
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ShockingCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.ShockingDamage, 1.0F) * WandUtil.damageMultiply();
        int range = spellStat.getRange();
                ...
                if (livingEntity.hurt(ModDamageSource.directShock(caster), damage)) {
                    ...
                    if (worldIn.random.nextFloat() <= chance) {
                        livingEntity.addEffect(
                                new MobEffectInstance(GoetiedEffects.SPASMS, MathHelper.secondsToTicks(5)));
                    }
                    if (burning > 0) {
