...
import za.co.infernos.goetied.client.particles.SparkleParticleOption;
...
public class VoidShock extends SpellEntity {
    ...
    public float baseDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.VoidShockDamage, 1.0F) * WandUtil.damageMultiply();
    public int life;
            ...
            if (entity1 instanceof LivingEntity living) {
                ...
                if (EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(living)) {
                    if (living.level().getRandom().nextFloat() <= 0.25F) {
                        if (!living.hasEffect(GoetiedEffects.VOID_TOUCHED)) {
                            living.addEffect(new MobEffectInstance(GoetiedEffects.VOID_TOUCHED, MathHelper.secondsToTicks(3), 2, false, true));
                        }
                    }
                }
            }
