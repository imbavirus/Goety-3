    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        if (entityIn instanceof Player player) {
            if (CuriosFinder.hasWindyRobes(player) && !player.isSpectator()){
                if (SEHelper.getSoulsAmount(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.WindRobeSouls, 0))
                        || player.isCreative()) {
                    ...
                    if (!player.onGround() && vector3d.y < 0.0D
                        ...
                        if (player.tickCount % 20 == 0 && !player.isCreative() && player.fallDistance > 3.0F) {
                            SEHelper.decreaseSouls(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.WindRobeSouls, 0));
                        }
        ...
        if (IronLoaded.IRON_SPELLBOOKS.isLoaded()){
            if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(MainConfig.RobesIronResist, false)) {
                if (stack.is(ModItems.STORM_ROBE.get())){
                    map.put(IronAttributes.LIGHTNING_MAGIC_RESIST, new AttributeModifier(Goetied.location("robes_iron_spell_resist"), 0.5F, AttributeModifier.Operation.ADD_VALUE));
                }
            }
        }
        return map;
