...
public class MagmaBomb extends SpellThrowableProjectile {
    ...
    public void tick() {
        ...
        this.level().addParticle(ModParticleTypes.BIG_FIRE.get(), d0 + this.level().random.nextDouble()/2, d1 + 0.5D, d2 + this.level().random.nextDouble()/2, 0.0D, 0.0D, 0.0D);
    }
    ...
    public void explode(HitResult pResult){
        if (!this.level().isClientSide) {
            ...
            MobUtil.explosionDamage(this.level(), this.getOwner() != null ? this.getOwner() : this, this.damageSources().explosion(this, this.getOwner()), vec3.x, vec3.y, vec3.z, this.explosionPower, 0);
            ...
            for (int i1 = 0; i1 < i; i1++) {
                ...
                if (magmaCube != null) {
                    ...
                    magmaCube.finalizeSpawn((ServerLevelAccessor) this.level(), this.level().getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.MOB_SUMMONED, null);
