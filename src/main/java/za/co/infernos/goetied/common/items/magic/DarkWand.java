...
import za.co.infernos.goetied.api.items.magic.IFocus;
...
import za.co.infernos.goetied.common.blocks.BrewCauldronBlock;
...
import za.co.infernos.goetied.common.entities.neutral.AbstractVine;
...
import za.co.infernos.goetied.utils.ConfigHelper;
...
public class DarkWand extends Item implements IWand {
    ...
    public InteractionResult useOn(UseOnContext pContext) {
        ...
        if (player != null) {
            ...
            if (focus != null && !focus.isEmpty() && focus.getItem() instanceof RecallFocus) {
                ...
                ISpell spell2 = GoetiedEventFactory.onBlockBasedSpell(player.level(), blockpos, player.level().getBlockState(blockpos), blockSpell0, pContext.getClickedFace(), player);
                ...
                if (!level.isClientSide) {
                    ...
                    if (bannerBlock.getBaseColor() != null) {
                        ...
                        player.displayClientMessage(Component.translatable("info.goetied.banner.add", player.getDisplayName()), true);
    ...
    public void onUseTick(Level worldIn, LivingEntity livingEntityIn, ItemStack stack, int count) {
        ISpell iSpell = GoetiedEventFactory.onStartSpell(livingEntityIn, stack, this.getSpell(stack));
        ...
        if (livingEntityIn.getUseItem() == stack && iSpell != null && this.isNotInstant(iSpell, livingEntityIn, stack)) {
            ...
            if (worldIn instanceof ServerLevel serverLevel) {
                iSpell = GoetiedEventFactory.onCastingSpell(livingEntityIn, stack, iSpell, castTime);
                if (iSpell != null) {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity livingEntity, int useTimeRemaining) {
        if (level instanceof ServerLevel serverLevel) {
            ...
            ISpell spell = GoetiedEventFactory.onStopSpell(livingEntity, stack, this.getSpell(stack), castTime, useTimeRemaining);
            if (spell != null) {
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level worldIn, LivingEntity entityLiving) {
        ...
        ISpell iSpell = GoetiedEventFactory.onCastSpell(entityLiving, this.getSpell(stack));
        if (iSpell != null) {
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (spell != null) {
            if (this.cannotCast(playerIn, itemstack)) {
                ...
                ISpell iSpell = GoetiedEventFactory.onCastSpell(playerIn, spell);
    ...
    public boolean canCastTouch(ItemStack stack, Level worldIn, LivingEntity caster) {
        ...
        ISpell spell = GoetiedEventFactory.onTouchBasedSpell(caster, stack, this.getSpell(stack));
    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        Player player = Goetied.PROXY.getPlayer();
        if (hasState(stack)) {
            int soulUse = getInt(stack, SOULUSE);
            tooltip.add(Component.translatable("info.goetied.wand.cost", soulUse));
            ...
            if (spell != null) {
                if (this.isNotInstant(spell, player, stack) && !(spell instanceof IChargingSpell)) {
                    ...
                    tooltip.add(Component.translatable("info.goetied.wand.castTime", castTime / 20.0F));
                }
                if (player != null && spell.spellCooldown(player) > 0) {
                    tooltip.add(Component.translatable("info.goetied.wand.coolDown", spell.spellCooldown(player) / 20.0F));
                }
            }
        } else {
            tooltip.add(Component.translatable("info.goetied.wand.cost", SoulCost(stack)));
        }
        ItemStack focus = IWand.getFocus(stack);
        if (focus != null && !focus.isEmpty()) {
            tooltip.add(Component.translatable("info.goetied.wand.focus", focus.getItem().getDescription()));
            ...
            tooltip.add(Component.translatable("info.goetied.wand.focus", Component.translatable("info.goetied.wand.empty")));
            if (ModKeybindings.wandSlot() != null) {
                tooltip.add(Component.translatable("info.goetied.wand.open", ModKeybindings.wandSlot().getTranslatedKeyMessage()).withStyle(ChatFormatting.BLUE));
            }
        }
        if (ModKeybindings.wandCircle() != null) {
            tooltip.add(Component.translatable("info.goetied.wand.switch", ModKeybindings.wandCircle().getTranslatedKeyMessage()).withStyle(ChatFormatting.BLUE));
        }
    }
