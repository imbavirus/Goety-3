    ...
    public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag flagIn) {
        if (stack.has(DataComponents.CUSTOM_DATA)) {
            ...
            if (tag.contains(TAG_PLAYER_NAME)) {
                tooltip.add(Component.translatable("tooltip.goetied.arca_compass_track", tag.getString(TAG_PLAYER_NAME)).withStyle(ChatFormatting.AQUA));
            }
            GlobalPos globalPos = getArcaPosition(tag);
            if (globalPos != null) {
                BlockPos blockPos = globalPos.pos();
                tooltip.add(Component.translatable("tooltip.goetied.arca").withStyle(ChatFormatting.GOLD)
                        .append(Component.translatable("tooltip.goetied.arcaCoords", blockPos.getX(), blockPos.getY(), blockPos.getZ())));
                if (getArcaDimension(tag).isPresent()){
                    ResourceKey<Level> dimension = getArcaDimension(tag).get();
                    tooltip.add(Component.translatable("tooltip.goetied.arcaDimension", dimension.location().toString()));
                }
            }
        } else {
            tooltip.add(Component.translatable("tooltip.goetied.arca_compass").withStyle(ChatFormatting.GOLD));
        }
