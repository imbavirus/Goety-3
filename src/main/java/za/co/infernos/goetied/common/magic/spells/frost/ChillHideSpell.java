...
public class ChillHideSpell extends Spell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ChillingCost, 0);
    }

    @Override
    public int defaultCastDuration() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ChillingDuration, 0);
    }
    ...
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.ChillingCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (isShifting(caster) && target != null){
            if (MobUtil.areAllies(target, caster)){
                target.addEffect(new MobEffectInstance(GoetiedEffects.CHILL_HIDE, MathHelper.secondsToTicks(45 * duration), potency));
                aabb = target.getBoundingBox().inflate(4.0D);
            }
        } else {
            caster.addEffect(new MobEffectInstance(GoetiedEffects.CHILL_HIDE, MathHelper.secondsToTicks(45 * duration), potency));
        }
        if (this.rightStaff(staff)){
            for (LivingEntity livingEntity : worldIn.getEntitiesOfClass(LivingEntity.class, aabb)) {
                if (MobUtil.areAllies(livingEntity, caster) && livingEntity != caster) {
                    livingEntity.addEffect(new MobEffectInstance(GoetiedEffects.CHILL_HIDE, MathHelper.secondsToTicks(45 * duration), potency));
                }
            }
        }
