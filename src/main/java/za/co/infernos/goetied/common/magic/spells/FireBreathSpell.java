...
public class FireBreathSpell extends BreathingSpell {
    ...
    public float getDamage() {
        if (damageCache < 0) {
            damageCache = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.FireBreathDamage, 1.0F) * WandUtil.damageMultiply();
        }
        return damageCache;
    }
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FireBreathCost, 0);
    }

    @Override
    public int defaultCastUp() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FireBreathChargeUp, 0);
    }

    @Override
    public int shotsNumber() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FireBreathDuration, 0);
    }

    @Override
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.FireBreathCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat) {
        ...
        if (!worldIn.isClientSide) {
            if (CuriosFinder.hasCurio(caster, ModItems.RING_OF_THE_DRAGON.get())) {
                damage *= 2.0F;
                if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(SpellConfig.DragonFireGriefing, false)) {
