package za.co.infernos.goetied.common.entities.ai;
...
import za.co.infernos.goetied.utils.WitchBarterHelper;
...
public class WitchBarterGoal extends Goal {
        ...
        if (this.progress <= 0) {
            ...
            if (!this.witch.level().isClientSide) {
                if (this.witch.level().getServer() != null) {
                    ...
                    ResourceKey<LootTable> loottableKey = ModLootTables.WITCH_BARTER;
                    if (this.witch instanceof Warlock) {
                        loottableKey = ModLootTables.WARLOCK_BARTER;
                    }
                    if (this.witch instanceof Heretic) {
                        loottableKey = ModLootTables.HERETIC_BARTER;
                    }
                    if (this.witch instanceof Crone) {
                        loottableKey = ModLootTables.CRONE_BARTER;
                    }
                    LootTable loottable = this.witch.level().getServer().reloadableRegistries()
    ...
    protected void addParticlesAroundSelf(ParticleOptions p_35288_) {
        if (!this.witch.level().isClientSide) {
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                serverLevel.sendParticles(p_35288_, this.witch.getRandomX(1.0D), this.witch.getRandomY() + 1.0D,
                        this.witch.getRandomZ(1.0D), 0, d0, d1, d2, 0.5F);
            }
        }
    ...
    public boolean canUse() {
        return this.witch.getMainHandItem().is(ModTags.Items.WITCH_CURRENCY)
                || this.witch.getMainHandItem().is(ModTags.Items.WITCH_BETTER_CURRENCY);
    }
