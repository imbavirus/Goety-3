...
public class BioMine extends SpellEntity {
    ...
    public void detonate() {
        if (!this.level().isClientSide) {
            ...
            float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.BiomineAcidDamage, 1.0F) * WandUtil.damageMultiply();
                ...
                if (livingentity.isAlive() && !livingentity.isInvulnerable()) {
                    if (this.getOwner() != null) {
                        if (!MobUtil.areAllies(this.getOwner(), livingentity) && livingentity != this.getOwner()) {
                            ...
                            livingentity.addEffect(new MobEffectInstance(GoetiedEffects.ACID_VENOM, 100, 1), this);
                        }
                    } else {
                        livingentity.hurt(ModDamageSource.acid(this, this.getOwner()), 0.0F);
                        livingentity.addEffect(new MobEffectInstance(GoetiedEffects.ACID_VENOM, 100, 1), this);
    ...
    public void explodeDamage(LivingEntity livingEntity) {
        if (!this.level().isClientSide) {
            float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.BiomineDamage, 1.0F) * WandUtil.damageMultiply();
            damage += this.getExtraDamage();
            if (this.getOwner() != null){
