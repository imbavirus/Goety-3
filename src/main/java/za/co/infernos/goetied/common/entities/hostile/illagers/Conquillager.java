...
public class Conquillager extends HuntingIllagerEntity implements CrossbowAttackMob {
    ...
    public static AttributeSupplier.Builder setCustomAttributes(){
        return Mob.createMobAttributes()
                ...
                .add(Attributes.MAX_HEALTH, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ConquillagerHealth, 20.0D))
                .add(Attributes.ARMOR, za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ConquillagerArmor, 20.0D))
                .add(Attributes.MOVEMENT_SPEED, 0.35D)
                .add(Attributes.ATTACK_DAMAGE, 5.0D);
    }

    public void setConfigurableAttributes(){
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.MAX_HEALTH), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ConquillagerHealth, 20.0D));
        MobUtil.setBaseAttributes(this.getAttribute(Attributes.ARMOR), za.co.infernos.goetied.utils.ConfigHelper.getDouble(AttributesConfig.ConquillagerArmor, 20.0D));
    }
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(8.0D), EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
            if (entity.isAlive() && !(entity instanceof PatrollingMonster) && !(entity instanceof RaiderServant) && !entity.getType().is(net.minecraft.tags.EntityTypeTags.UNDEAD)) {
                if (this.tickCount % 100 == 0 && this.getRandom().nextInt(20) == 0) {
                    entity.addEffect(new MobEffectInstance(GoetiedEffects.ILLAGUE, 2000, 0, false, false));
                }
            }
        }
    ...
    public static class IllagerCrossbowGoal<T extends PathfinderMob & RangedAttackMob & CrossbowAttackMob> extends Goal {
        private final T mob;
