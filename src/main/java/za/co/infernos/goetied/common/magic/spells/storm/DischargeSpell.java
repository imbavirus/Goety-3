...
public class DischargeSpell extends Spell {
    ...
    public int defaultSoulCost() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.DischargeCost, 0);
    }

    @Override
    public int defaultCastDuration() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.DischargeDuration, 0);
    }

    @Override
    public int defaultSpellCooldown() {
        return za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.DischargeCoolDown, 0);
    }
    ...
    public void SpellResult(ServerLevel worldIn, LivingEntity caster, ItemStack staff, SpellStat spellStat){
        ...
        float damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.DischargeDamage, 1.0F) * WandUtil.damageMultiply();
        float maxDamage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.DischargeMaxDamage, 1.0F) * WandUtil.damageMultiply();
            ...
            public void explodeHurt(Entity target, DamageSource damageSource, double x, double y, double z, double seen, float actualDamage) {
                if (target instanceof LivingEntity target1){
                    ...
                    if (worldIn.random.nextFloat() <= chance){
                        target1.addEffect(new MobEffectInstance(GoetiedEffects.SPASMS, MathHelper.secondsToTicks(5)));
                    }
                    if (rightStaff(staff)){
