...
import za.co.infernos.goetied.client.particles.AuraParticle;
...
public class GoetiedBaseEffect extends MobEffect {
    public GoetiedBaseEffect(MobEffectCategory p_19451_, int p_19452_) {
    ...
    public boolean applyEffectTick(LivingEntity livingEntity, int amplify) {
        ...
        if (this == GoetiedEffects.PRESSURE.get()) {
            int count = 0;
            for (LivingEntity other : world.getEntitiesOfClass(LivingEntity.class,
                    livingEntity.getBoundingBox().inflate(8.0F + amplify))) {
        ...
        if (this == GoetiedEffects.STORMS_WRATH.get()) {
            float chance = 0.1F;
        ...
        if (this == GoetiedEffects.PHOTOSYNTHESIS.get()) {
        ...
        if (this == GoetiedEffects.TRIPPING.get()) {
        ...
        if (this == GoetiedEffects.ARROWMANTIC.get()) {
            for (AbstractArrow abstractArrow : world.getEntitiesOfClass(AbstractArrow.class,
                    livingEntity.getBoundingBox().inflate(2.0F + amplify))) {
        ...
        if (this == GoetiedEffects.FIERY_AURA.get()) {
            if (world instanceof ServerLevel serverLevel) {
                float f = 2.0F + amplify;
                ServerParticleUtil.addAuraParticles(serverLevel, ParticleTypes.FLAME, livingEntity, f);
                for (LivingEntity living : world.getEntitiesOfClass(LivingEntity.class,
                        livingEntity.getBoundingBox().inflate(f))) {
                    if (!living.isOnFire() && !living.hasEffect(MobEffects.FIRE_RESISTANCE) && !living.fireImmune()
                            && !MobUtil.areAllies(livingEntity, living) && MobUtil.validEntity(living)
                            && living != livingEntity) {
                        ...
                        ModNetwork.sendToALL(new SPlayWorldSoundPacket(livingEntity.blockPosition(),
                                SoundEvents.FIRECHARGE_USE, 1.0F, 0.75F));
                        living.igniteForSeconds(5 * (amplify + 1));
                    }
                }
            }
        }
        if (this == GoetiedEffects.FROSTY_AURA.get()) {
            if (world instanceof ServerLevel serverLevel) {
                float f = 2.0F + amplify;
                ServerParticleUtil.addAuraParticles(serverLevel, ParticleTypes.SNOWFLAKE, livingEntity, f);
                for (LivingEntity living : world.getEntitiesOfClass(LivingEntity.class,
                        livingEntity.getBoundingBox().inflate(f))) {
                    if (!living.isFreezing() && living.canFreeze() && !MobUtil.areAllies(livingEntity, living)
                            && MobUtil.validEntity(living) && living != livingEntity) {
                        ...
                        living.addEffect(new MobEffectInstance(GoetiedEffects.FREEZING, 100, amplify));
                    }
                }
            }
        }
        if (this == GoetiedEffects.CHILL_HIDE.get()) {
            if (world instanceof ServerLevel serverLevel) {
                if (livingEntity.tickCount % 5 == 0) {
                    float f = livingEntity.getBbWidth();
                    ServerParticleUtil.windParticle(serverLevel, new ColorUtil(0x7eb7d4), f,
                            livingEntity.getBbHeight() / 2.0F, livingEntity.getId(), livingEntity.position());
                }
                if (livingEntity.tickCount % 15 == 0) {
                    serverLevel.sendParticles(
                            new AuraParticle.Option(livingEntity.getId(), livingEntity.getBbHeight(),
                                    new ColorUtil(MapColor.ICE)),
                            livingEntity.getX(), livingEntity.getY() + (livingEntity.getBbHeight() / 2.0F),
                            livingEntity.getZ(), 1, 0, 0, 0, 0.5F);
                }
            }
        }
        if (this == GoetiedEffects.FIRE_TRAIL.get()) {
            BlockState blockState = world.getBlockState(livingEntity.blockPosition());
            if (MobUtil.isMoving(livingEntity)) {
                if (blockState
                        .canBeReplaced(new DirectionalPlaceContext(world, livingEntity.blockPosition(), Direction.DOWN,
                                ItemStack.EMPTY, Direction.UP))
                        && world.getFluidState(livingEntity.blockPosition()).isEmpty()
                        && world.getBlockState(livingEntity.blockPosition().below()).isSolidRender(world,
                                livingEntity.blockPosition().below())) {
                    world.setBlockAndUpdate(livingEntity.blockPosition(),
                            BaseFireBlock.getState(world, livingEntity.blockPosition()));
                }
            }
        }
        if (this == GoetiedEffects.GRAVITY_PULSE.get()) {
        ...
        if (this == GoetiedEffects.PLUNGE.get()) {
                ...
                if (MobUtil.validEntity(livingEntity)) {
                    ...
                    if (livingEntity.isInFluidType()) {
                        livingEntity.setDeltaMovement(
                                livingEntity.getDeltaMovement().subtract(0, 0.05D + (amplify / 100.0D), 0));
                    } else if (BlockFinder.distanceFromGround(livingEntity) > 4) {
                        livingEntity.setDeltaMovement(
                                livingEntity.getDeltaMovement().subtract(0, 0.2D + (amplify / 10.0D), 0));
                    }
                    livingEntity.setSwimming(false);
                }
            }
        }
        if (this == GoetiedEffects.WOUNDED.get()) {
        ...
        if (this == GoetiedEffects.CRIPPLED.get()) {
            if (livingEntity.getHealth() >= livingEntity.getMaxHealth() * 0.75F) {
                livingEntity.removeEffect(GoetiedEffects.CRIPPLED);
            }
        }
        if (this == GoetiedEffects.STUNNED.get()) {
        ...
        if (this == GoetiedEffects.WILD_RAGE.get()) {
            if (!livingEntity.level().isClientSide) {
                if (livingEntity instanceof Mob mob
                    ...
                    LivingEntity target = world.getNearestEntity(world.getEntitiesOfClass(LivingEntity.class,
                            mob.getBoundingBox().inflate(follow, 4.0D, follow), (p_148152_) -> {
                                return true;
    ...
    public boolean shouldApplyEffectTickThisTick(int tick, int amplify) {
        if (this == GoetiedEffects.NYCTOPHOBIA.get()
                || this == GoetiedEffects.PRESSURE.get()) {
            int j = 40 >> amplify;
            if (j > 0) {
                return tick % j == 0;
            }
        ...
        if (this == GoetiedEffects.TRIPPING.get()
                || this == GoetiedEffects.STORMS_WRATH.get()) {
            int j = 20 >> amplify;
            if (j > 0) {
                return tick % j == 0;
            }
