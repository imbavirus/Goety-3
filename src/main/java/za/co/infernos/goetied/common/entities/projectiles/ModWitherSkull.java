      ...
      if (!this.level().isClientSide) {
         ...
         if (entity1 instanceof LivingEntity livingentity) {
            if (entity1 instanceof Player){
               damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.WitherSkullDamage, 1.0F) * WandUtil.damageMultiply();
            } else {
         ...
         if (flag && entity instanceof LivingEntity livingEntity) {
          ...
          if (this.getOwner() instanceof Player) {
             explodeMode = za.co.infernos.goetied.utils.ConfigHelper.getBoolean(SpellConfig.WitherSkullGriefing, false) ? Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.KEEP;
          }
