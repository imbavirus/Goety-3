                ...
                if (MobUtil.isShifting(player)) {
                    if (SEHelper.addAllyEntityType(player, pTarget.getType())) {
                        if (SEHelper.getGrudgeEntityTypes(player).contains(pTarget.getType())){
                            ...
                            player.displayClientMessage(Component.translatable("info.goetied.grudge.removeType", I18n.get(pTarget.getType().getDescriptionId())), true);
                        }
                        pTarget.playSound(SoundEvents.PLAYER_LEVELUP, 1.0F, 0.25F);
                        player.displayClientMessage(Component.translatable("info.goetied.grimoire.addType", I18n.get(pTarget.getType().getDescriptionId())), true);
                    }
                } else {
                    if (SEHelper.addAllyEntity(player, pTarget)) {
                        if (SEHelper.getGrudgeEntities(player).contains(pTarget)){
                            ...
                            player.displayClientMessage(Component.translatable("info.goetied.grudge.remove", I18n.get(pTarget.getType().getDescriptionId())), true);
                        }
                        pTarget.playSound(SoundEvents.PLAYER_LEVELUP);
                        player.displayClientMessage(Component.translatable("info.goetied.grimoire.add", pTarget.getDisplayName()), true);
                    }
                }
            }
        }
        ...
        if (!level.isClientSide) {
            if (MobUtil.isShifting(player)) {
                if (SEHelper.getAllyEntityTypes(player).isEmpty()){
                    player.displayClientMessage(Component.translatable("info.goetied.goodwill.emptyTypes"), false);
                } else {
                    player.displayClientMessage(Component.translatable("info.goetied.goodwill.type"), false);
                ...
                if (SEHelper.getAllyEntities(player).isEmpty()){
                    player.displayClientMessage(Component.translatable("info.goetied.goodwill.empty"), false);
                } else {
                    player.displayClientMessage(Component.translatable("info.goetied.grimoire.goodwill"), false);
