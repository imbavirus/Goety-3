...
public class Pyroclast extends ThrowableProjectile implements ISpellEntity {
    ...
    public void explode() {
        if (!this.level().isClientSide) {
            ...
            if (owner instanceof Player) {
                if (!za.co.infernos.goetied.utils.ConfigHelper.getBoolean(SpellConfig.PyroclastGriefing, false)) {
                    flag = false;
                }
            }
    ...
    public void hitEntity(Entity entity) {
        ...
        if (entity1 instanceof Player) {
            damage = za.co.infernos.goetied.utils.ConfigHelper.getFloat(SpellConfig.PyroclastDamage, 1.0F) * WandUtil.damageMultiply();
        }
