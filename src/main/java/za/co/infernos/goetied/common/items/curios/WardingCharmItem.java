...
import za.co.infernos.goetied.common.magic.spells.IronHideSpell;
...
public class WardingCharmItem extends SingleStackItem{
    ...
    public boolean SoulCostUp(LivingEntity entityLiving){
        return entityLiving.hasEffect(GoetiedEffects.SUMMON_DOWN);
    }

    public int SoulCalculation(LivingEntity entityLiving){
        if (SoulCostUp(entityLiving)){
            int amp = Objects.requireNonNull(entityLiving.getEffect(GoetiedEffects.SUMMON_DOWN)).getAmplifier() + 2;
        ...
        if (entityIn instanceof LivingEntity livingEntity) {
            ...
            if (!worldIn.isClientSide) {
                if (livingEntity instanceof Player player) {
                    if (!player.hasEffect(GoetiedEffects.SOUL_ARMOR)) {
                        if (SEHelper.getSoulsAmount(player, SoulUse(player, stack))) {
                            ...
                            if (hostile != null || player.hurtTime > 0) {
                                ...
                                player.addEffect(new MobEffectInstance(GoetiedEffects.SOUL_ARMOR, MathHelper.minutesToTicks(duration), enchantment, false, false, true));
