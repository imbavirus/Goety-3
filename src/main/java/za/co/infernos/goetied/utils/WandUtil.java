...
import za.co.infernos.goetied.common.entities.neutral.TotemicBomb;
import za.co.infernos.goetied.common.entities.projectiles.Fangs;
import za.co.infernos.goetied.common.entities.projectiles.IceBouquet;
import za.co.infernos.goetied.common.entities.projectiles.Spike;
import za.co.infernos.goetied.common.entities.util.TridentStorm;
...
import za.co.infernos.goetied.common.network.server.SThunderBoltPacket;
import za.co.infernos.goetied.config.SpellConfig;
import za.co.infernos.goetied.init.ModAttributes;
...
public class WandUtil {
    ...
    public static float damageMultiply() {
        return (float) (za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.SpellDamageMultiplier, 1) * za.co.infernos.goetied.utils.ConfigHelper.getDouble(SpellConfig.SpellDamageMultiplierDecimal, 1.0D));
    }

    public static int getPotencyLevel(LivingEntity livingEntity) {
        return getLevels(ModEnchantments.POTENCY.get(), livingEntity) * za.co.infernos.goetied.utils.ConfigHelper.getInt(SpellConfig.PotencyPower, 1);
    }
    ...
    public static void summonTurret(LivingEntity casterEntity, BlockPos targetPos, EntityType<? extends AbstractMonolith> wallEntityType, @Nullable Entity target, int delay, int duration, int potency) {
        ...
        if (monolith != null) {
            ...
            if (monolith != null) {
                ...
                if (potency > 0 && !casterEntity.hasEffect(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.SUMMON_DOWN.get()))) {
                    monolith.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.BUFF.get()), EffectsUtil.infiniteEffect(), potency - 1, false, false));
                }
