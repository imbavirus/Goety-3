...
public class ItemHelper {
    ...
    public static void repairTick(ItemStack stack, Entity entityIn, boolean isSelected){
        if (za.co.infernos.goetied.utils.ConfigHelper.getBoolean(ItemConfig.SoulRepair, false)) {
            if (entityIn instanceof Player player) {
                if (!(player.swinging && isSelected)) {
                    if (stack.isDamaged()) {
                        if (SEHelper.getSoulsContainer(player)){
                            int i = 1;
                            if (SEHelper.getSoulsAmount(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.ItemsRepairAmount, 0) * i)){
                                if (player.tickCount % 20 == 0) {
                                    stack.setDamageValue(stack.getDamageValue() - 1);
                                    SEHelper.decreaseSouls(player, za.co.infernos.goetied.utils.ConfigHelper.getInt(ItemConfig.ItemsRepairAmount, 0) * i);
                                }
                            }
                        }
                    }
    ...
    public static void setItemEffect(ItemStack stack, LivingEntity victim){
        if (stack.getItem() instanceof TieredItem weapon){
            if (weapon.getTier() == ModTiers.DARK) {
                victim.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.WANE.get()), 60));
            }
            if (weapon == ModItems.FELL_BLADE.get() && victim.getRandom().nextBoolean()) {
                victim.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.BUSTED.get()), MathHelper.secondsToTicks(5)));
            }
            if (weapon == ModItems.FROZEN_BLADE.get()) {
                victim.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.FREEZING.get()), MathHelper.secondsToTicks(2)));
            }
        } else if (stack.getItem() instanceof PhilosophersMaceItem){
            int i2 = 0;
            victim.addEffect(new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(GoetiedEffects.GOLD_TOUCHED.get()), 300, i2));
        }
    }
    ...
    public static Component getShiftInfoTooltip() {
        ...
        return Component.translatable("info.goetied.item_info", shift).withStyle(ChatFormatting.GRAY);
    }
